...
| folklore | `is broken`, `does not work`, `не работает`, `сломан` anywhere in the text (frontmatter included), case-insensitive, reported with the line number. `never use` is **not** folklore: whether a prohibition is evidence-backed is the reviewer's check (`unsafe_claim`) |
