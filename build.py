...
class BuildManager:
    """
    统一构建管理：依赖同步 → 构建 / 测试 → 归档。

    用法:
        python build.py                             完整构建（同步构建依赖并生成 wheel）
        python build.py local                       本地构建（跳过依赖同步）
        python build.py --version/-v <version>      指定构建版本号
        python build.py --extra/-e KEY=VALUE        传递额外构建环境变量，可多次使用
        python build.py test                        同步测试依赖并运行 UT 和 Skill 用例
        python build.py test local                  跳过依赖同步并运行 UT 和 Skill 用例

    参数说明:
        - 参数: command    : 构建动作: 为空时为全构建, local 为跳过依赖同步, test 为运行单元测试。
    ...
    def __init__(self):
        ...
        self._validate_commands(ap)

    def _validate_commands(self, parser):
        commands = self.args.command
        allowed = {"local", "test"}
        invalid = set(commands) - allowed
        if invalid or len(commands) != len(set(commands)):
            parser.error("COMMAND may contain each of 'local' and 'test' at most once")
    ...
    def _build_environment(self):
        ...
        if "local" in self.args.command:
            # scripts/run_ut.sh uses uv run, which otherwise synchronizes the environment.
            env["UV_NO_SYNC"] = "1"
    ...
    def _sync_dependencies(self, dependency_mode):
        if "local" in self.args.command:
            logging.info("Skipping dependency synchronization for local build.")
            return
        self._execute_command(["uv", "sync", "--locked", dependency_mode], cwd=self.project_root)
    ...
    def run(self):
        ...
        if "test" in self.args.command:
            # -------------------- 单元测试 --------------------
            self._sync_dependencies("--dev")
            self._run_tests(env)
        else:
            # -------------------- 产品构建 --------------------
            self._sync_dependencies("--no-dev")
