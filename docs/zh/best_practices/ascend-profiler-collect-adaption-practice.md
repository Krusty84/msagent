...
以强化学习场景的 `slime-ascend` 框架为例，展示如何通过 msagent 的 `ascend-profiler-collect-adaption` Skill 端到端完成 `slime-ascend` 框架的 `torch_npu.profiler` 接口适配。
...
3. 强化学习框架包含rollout和actor阶段，则将 `torch_npu.profiler` 接口分别适配到rollout和actor阶段中，分别控制各阶段的采集，避免跨阶段串扰。
