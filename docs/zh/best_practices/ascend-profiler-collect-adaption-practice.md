...
以推理场景的 `sglang` 框架为例，展示如何通过 msagent 的 `ascend-profiler-collect-adaption` Skill 端到端完成 `sglang` 框架的 `torch_npu.profiler` 接口适配。
...
## 场景 2：训练场景 `slime-ascend` 框架适配 `torch_npu.profiler` 接口

以训练场景的 `slime-ascend` 框架为例，展示如何通过 msagent 的 `ascend-profiler-collect-adaption` Skill 端到端完成 `slime-ascend` 框架的 `torch_npu.profiler` 接口适配。

### 流程

1. 用户提供训练场景 `slime-ascend` 框架代码，并说明需要接入 `torch_npu.profiler`。
2. msagent 调用 `ascend-profiler-collect-adaption` Skill，定位训练场景的 profiler 注入位置。
3. 若训练框架包含分阶段执行逻辑，则将 profiler 参数内置到所属阶段参数中，分别控制各自阶段采集，避免跨阶段串扰。
