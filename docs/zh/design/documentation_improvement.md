...
开发者能够在独立接口说明中查询主命令、`config` 和 `web` 子命令的常用参数，了解交互式会话、单次请求、工作目录、Agent 选择、模型选择、审批模式、配置检查和 Web 服务等入口。完整参数仍以 `--help` 输出为准，文档不复制内部实现细节。
...
| 接口说明 | 覆盖主命令、`config` 与 `web` 常用参数 | 对照三组 `--help` 输出 |
...
- Quick Start 只描述最短可用路径，并链接到安装、FAQ 和贡献指南。
...
快速开始保留发布包安装作为默认路径，将源码运行作为贡献和本地验证补充。LLM 配置提供 OpenAI-compatible 示例，并通过 FAQ 说明 `export` 与 `.env` 的适用范围。配置是否成功统一使用 `msagent config --show` 检查，不打印真实 API Key。
...
**可验收设计**：按第 2.6 节验收矩阵执行，并记录构建结果、warning 基线和 PR 文档门禁状态。
...
| 外部链接失效 | 文档门禁失败或用户无法继续 | 优先使用仓库内相对链接，外链纳入有效性检查 |
...
- 文档门禁：检查 Markdown 规范、链接、资源和标签闭合。
...
当前没有待决策的设计问题；是否合入仍以维护者审查和仓库门禁结果为准。后续可由社区单独讨论：
...
uv run --with sphinx --with sphinx-rtd-theme --with myst-parser sphinx-build -E -b html docs docs/_build/html
