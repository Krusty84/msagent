...
msAgent 中的 Agent 采用声明式配置定义，无需编写 Python 代码。添加新 Agent 只需创建 YAML 配置文件和对应的 Prompt 文件即可。
...
配置文件存放在 `.msagent/agents/` 目录下，首次运行时会自动从 `resources/configs/default/` 拷贝模板。
...
在 `.msagent/agents/` 下创建 `MyAgent.yml`（文件名必须与 `name` 一致）：
...
在 `prompts/agents/` 下创建 `MyAgent.md`，定义 Agent 的系统提示词：
...
配置完成后，重新启动 `msagent`，通过以下方式验证：

```bash
# 直接指定 Agent 启动
msagent --agent MyAgent

# 或在会话中通过 /agents 命令切换到新 Agent
/agents
```

## 6. 打包到默认模板（可选）

如果希望新 Agent 随 `msagent init` 自动生成到用户的 `.msagent/` 目录，将配置文件放入源码 `resources/configs/default/` 的对应路径中。首次运行时 `msagent` 会自动将其拷贝到 `.msagent/` 下：

```text
resources/configs/default/          →  用户 .msagent/
├── agents/                         →  .msagent/agents/
│   └── MyAgent.yml                 →    MyAgent.yml
└── prompts/                        →  .msagent/prompts/
    └── agents/                     →    agents/
        └── MyAgent.md              →      MyAgent.md
```
