...
本文档描述 MindStudio-Agent 的整体架构设计。MindStudio-Agent 是一个面向 Ascend NPU 开发、调试和调优场景的 AI Agent 工作台，当前通过统一框架承载 Profiler、Accuracy、Quantizer、Modeling、Operator、Minos 等多个专业化 Agent，并支持 CLI 会话与可选 Web 入口。
...
7. **交互界面**：提供默认 CLI 会话，以及面向 deep-agents-ui 的可选 Web 入口
...
│                        交互层                                │
│  ┌──────────────────┐  ┌────────────────────────────────┐   │
│  │ CLI 会话 (msagent)│  │ Web 入口 (msagent web + UI)   │   │
│  └──────────────────┘  └────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
...
- **职责**：
  - 提供默认命令行会话入口，以及 `config` / `web` 等公开命令
  - 处理用户输入、线程切换、工具展示、补全和快捷键等交互增强功能
  - 在 Web 模式下导出 LangGraph graph，并对接 deep-agents-ui
...
sequenceDiagram
    ...
    participant Entry as CLI/Web入口
    ...
    User->>Entry: 启动 (msagent / msagent web)
    ...
    Entry->>Graph: 进入 CLI 会话或导出 Web Graph
```
...
| langgraph-cli[inmem]（可选） | Web 运行入口 | 支持 `msagent web` 场景 |
...
3. **启动 Web 入口（可选）**：
   ...
   ```bash
   msagent web --agent Profiler
   ```

4. **使用限制**：
   - 不同 Agent 有不同的领域定位，使用时需选择合适的 Agent
   - Web 模式依赖可用的 LangGraph CLI 运行环境
