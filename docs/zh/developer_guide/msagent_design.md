...
| -- | -- | -- | -- | -- |
| 2026-06-03 | 1.0 | 补充 msAgent 详细设计文档，覆盖架构、交互链路、扩展机制与测试设计 | kali20gakki1 | https://deepwiki.com/kali20gakki/msAgent |
|  |  |  |  |  |
...
`msAgent` 是一个面向 Ascend NPU 场景的一站式调试调优 Agent 框架。它不是单一能力点工具，而是将以下能力整合为统一的交互式工作台：
...
`msAgent` 的核心目标不是“替代专家”，而是把领域知识、工具调用、上下文管理和交互体验组合成一个可重复使用、可扩展、可维护的 Agent 平台。

### 2. 业务痛点

Ascend 生态下的调试调优存在几个典型难点：

- 问题域多样：性能、精度、量化、算子优化、文档体验等问题需要不同知识体系。
- 工具链分散：本地 shell、MCP 服务、网页检索、技能脚本、配置模板往往分散在多个入口。
...
这一层更适合用“**能力编排视角**”理解，而不是界面示意。`msAgent` 的默认 Agent 不是靠名称区分，而是靠“问题域 -> Prompt 约束 -> Tool 边界 -> Skill 组合 -> SubAgent 协作方式”共同定义其行为。
...
从设计上看，5 个默认 Agent 共用同一套运行时骨架，但在以下维度上各自收敛：
...
因此，这一层的设计重点是通过配置把不同问题域映射成不同的**分析策略与能力边界**。这也是后续新增 Agent 时应保持的扩展方式：优先新增 Prompt、Skill、Tool Pattern 和 SubAgent 组合，而不是复制一套新的运行时实现。

### 6. 配置系统设计

#### 6.1 配置源与优先级

`msAgent` 的配置采用“**安装包默认模板 + 工作目录本地实例**”的方式组织：首次运行时，`ConfigRegistry.ensure_config_dir()` 会把安装包中的默认模板复制到当前工作目录下的 `.msagent/`，后续运行优先读取这个本地目录。
...
- **本地配置隔离**：敏感信息使用 `api_key_env` 与环境变量承载，不建议写死在仓库文件中。
...
- 长会话虽然支持压缩与检查点，但若外部工具本身输出异常巨大，仍建议通过 Tool Pattern 和问题分解控制范围。
...
对于后续演进，建议将以下检查作为最低回归门禁：

1. 配置相关变更：
   运行配置模型、版本兼容和 registry 相关单测。
2. 运行时/装配变更：
   运行 `AgentFactory`、`Initializer`、`MCPClient` 相关集成测试。
3. 交互变更：
   运行 CLI handler / renderer / completer 测试。
4. Web 变更：
   运行 `test_web_runtime.py`、`test_web_launcher.py`。
5. 发布前兜底：
   运行 E2E 入口测试。
...
其中 `docs-build` 对本次设计文档尤其重要，可防止 Mermaid 代码块、图片路径或 Markdown 语法在 Sphinx 构建时失效。

### 6. 本次设计文档相关测试重点

本篇设计文档本身虽然不改动运行时代码，但建议在后续围绕文档-实现一致性建立以下校验习惯：

- 文档中出现的命令、目录结构、配置键名与当前代码保持一致。
- 文档中出现的 Agent 名称、Skill 名称、MCP 默认服务与默认模板保持一致。
- 文档中描述的测试文件与仓库真实路径保持一致。
- 文档变更后执行一次 Sphinx HTML 构建，确保图片与 Mermaid 代码块可被正确处理。
