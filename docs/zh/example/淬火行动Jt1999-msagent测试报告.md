...
**判定**：4 卡计算时间几乎一致（332-335ms，算子对比差异 <2%），排除计算型慢卡与负载切分不均；慢卡空闲（Free）远超正常卡，指向**下发/调度瓶颈**。
...
→ 慢卡 CPU 侧每次下发 attention 融合算子更耗时，且偶发秒级大停顿（45.4ms）。
...
- 各 rank allReduce 累计（258 次）：rank0=425.9ms、rank3=424.6ms（以 idle 为主 420ms），rank1=200.2ms（以 wait 为主 194.9ms）
...
- 工具：`msprof-analyze` 8.5.2（cluster_time_summary / free_analysis / cann_api_sum / hccl_sum / compute_op_sum / slow_link / advisor）
