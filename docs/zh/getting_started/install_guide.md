...
默认构建会安装 `uv` 并同步依赖。如本地已安装 `uv`，可使用 `python3 build.py local` 跳过安装；该模式仍会同步依赖。需要指定构建版本或传递底层构建脚本支持的环境变量时，可使用：
...
该命令会安装 `uv`、同步测试依赖，并运行 `tests/ut` 和 `tests/skills` 下的用例。如本地已安装 `uv`，可执行 `python3 build.py test local` 跳过安装；该模式仍会同步测试依赖。命令返回码为 0，且测试用例均无失败，表示单元测试通过。
