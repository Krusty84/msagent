...
1. 准备一个可用的 LLM API Key。
   ...
   需要用户自行登录模型服务商网站进行创建，例如http://models.ascend.huawei.com。

2. 配置 LLM。

   包括 LLM 服务的环境变量（*_API_KEY）、通过`msagent config`命令的`--llm-provider`参数配置LLM 服务的协议类型、`--llm-base-url`参数配置模型服务商网站地址、`--llm-model`参数配置模型名称（模型名称从模型服务商网站的模型广场获取）。
...
- 启动并进入[Hermes 性能调优](../agent_guide/Hermes.md)功能交互式会话。
...
- 启动并进入[Zephyr 模型量化](../agent_guide/Zephyr.md)功能交互式会话。

  ```bash
  msagent --agent Zephyr
  ```

- 启动并进入[Icarus 算子调优](../agent_guide/Icarus.md)功能交互式会话。
