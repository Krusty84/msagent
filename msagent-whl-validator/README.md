...
│   ├── llm_capture_server.py     # 本地 OpenAI 兼容请求捕获服务
...
│   ├── test_02_skills.py         # Skill 路由、读取和动态加载占位
│   ├── test_03_sys_prompt.py     # Agent System Prompt 和环境信息
│   └── test_04_local_env.py      # Filesystem、LocalShell，占位未实现
...
MCP 验证用例当前不在工程中，待接口和测试输入明确后再单独加入。
...
# 只验证 Skill
"$ENV_PYTHON" -m pytest tests/test_02_skills.py

# System Prompt 使用本地捕获服务，不访问真实 LLM
"$ENV_PYTHON" -m pytest tests/test_03_sys_prompt.py
...
### 7.4 `test_04_local_env.py`

规划验证：

- Filesystem 能读取 workspace 中的 `read_marker.txt`。
- LocalShell 能执行 workspace 中的 `print_marker.sh`。
- trace 中包含正确的工具调用、成功退出码和唯一输出标记。

两个用例当前均为 skip 占位，尚未实现。
...
- MCP 工具验证。
- Filesystem 和 LocalShell 的具体执行断言。
