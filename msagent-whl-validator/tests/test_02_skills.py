...
from lib.agent_runner import RunResult
from lib.assertions import assert_session_succeeded
from lib.trace_parser import get_tool_calls, get_tool_results
...
def _assert_skill_invoked(
    ...
    target_calls = [
        ...
        and event.get("input", {}).get("category") == case.skill_category
    ]
    assert target_calls, (
...
@pytest.mark.skip(reason="待实现：验证运行中的交互会话动态发现外部复制的 Skill")
def test_running_interactive_session_discovers_copied_skill_without_restart() -> None:
    """外部复制 Skill 后，同一个交互式 msagent 进程应无需重启即可发现它。"""
