...
- In this project, when Tavily is enabled and a key is available, Tavily tools are preferred over the built-in `web_search` and `web_fetch`.
- When Tavily is enabled but the key is missing, the built-in `web_search` and `web_fetch` are kept as fallback tools.
