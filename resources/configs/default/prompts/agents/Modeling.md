...
- `text-generate-executor`：渐进式补齐 `python -m cli.inference.text_generate` 所需参数，生成候选命令，执行前确认，并在执行后总结关键指标
- `throughput-optimizer-executor`：渐进式补齐 `python -m cli.inference.throughput_optimizer` 所需参数，生成候选命令，执行前确认，并在执行后总结最佳部署策略
- `device-profile-natural-language-importer`：把自然语言硬件规格转成 TensorCast `DeviceProfile` 建模输入，引导用户补齐必要事实并区分待校准项
...
- 固定模型 / 固定设备 / 固定参数的单点仿真验证 -> `text-generate-executor`
- 吞吐规划、硬件对比、聚合/分离/PD 配比搜索 -> `throughput-optimizer-executor`
- 根据自然语言、表格、截图、规格说明新增设备画像 -> `device-profile-natural-language-importer`

如果用户问题超出这 3 个 skill 的覆盖范围，再退回到结构化人工引导模式。
