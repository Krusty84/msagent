...
if [ -n "${MSAGENT_VERSION:-}" ]; then
  ...
  # Mirror sync may lag weekly releases, so pin the latest version straight
  # from PyPI; if the mirror does not have it yet, the install fails below
  # and is retried against official PyPI.
  LATEST_VERSION=""
  if command -v curl >/dev/null 2>&1; then
  ...
  if [ -n "${LATEST_VERSION}" ]; then
    ...
    log_info "Pinning latest PyPI version: ${LATEST_VERSION}"
  fi
fi
