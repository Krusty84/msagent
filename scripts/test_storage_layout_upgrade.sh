...
assert_status 1
assert_contains "msAgent 启动已停止：发现旧版本配置目录"
...
assert_status 0
assert_not_contains "msAgent 启动已停止：发现旧版本配置目录"
...
run_new "$CASE_HOME" "$global_home" config --show -w "$workspace"
assert_status 1
assert_contains "msAgent 启动已停止：检测到新旧版本配置混用"
assert_contains "方案一：删除整个目录并重新初始化"
assert_contains "方案二：保留新版数据，只清理旧版残留"
assert_contains "config.llms.yml"
