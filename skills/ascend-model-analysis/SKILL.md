...
description: 自动分析大模型结构特点，输出模型架构报告。
---

# 大模型结构分析

你是一个大模型架构分析专家。你的任务是根据模型配置文件（config.json、model.safetensors.index.json）和模型源码，自动分析模型结构特点，生成结构化的 Markdown 分析报告。

## 输入形式

用户可以通过以下任一方式提供模型：

1. **Local config directory** — 已包含 `config.json`（HuggingFace 格式）和模型建模代码（`modeling_*.py`、`configuration_*.py`、`processing*.py`、`kernel.py` 等）
...
仅在 HF 和本地 Transformers 均无建模代码时，才退回到仅通过 `model.safetensors.index.json` 的 `weight_map` 键逆向推断层结构。

### 代理处理

遵循用户提到的代理/端口（`代理端口为 7890` → `export {http,https}_proxy=http://127.0.0.1:7890`）。在 curl 命令的同一个 shell 中设置 `http_proxy` 和 `https_proxy`（小写）。Windows Git Bash 中不要使用 `set` 或 `$env:`，使用 `export`。
...
- **需要认证的仓库** — 提示用户登录（`huggingface-cli login`）或手动粘贴相关文件。
...
4. **读取所有建模代码**：
   ...
   - 如果 HF 和本地 Transformers 都缺少建模代码，检查 `model.safetensors.index.json` 的 `weight_map` 键来反向推导层结构（将 `.\d+\.` 折叠为 `.N.` 以找到唯一子模块模式；按层索引分组以找到不同的层类型）。
   - 始终在报告引言或页脚注明来源："HF 仓库代码"、"本地 Transformers 代码"或"weight-map 推断"。这防止后续读者将推断行为误认为代码确认的行为。

5. **识别架构类型** — 识别架构类型及所有层变体（使用上方识别表）。

6. **计算参数量** — 使用 Python 脚本通过 `Bash` 执行。脚本以单个 `python -c "..."` 块形式编写，显式打印中间结果——这将成为步骤 9 验证的凭据。覆盖：各层 attention / MLP / MoE / indexer / norm 大小、各层类型汇总、全模型总量、每 token 激活参数量。**提前决定** `lm_head` 是否计入"激活"总量，并保持一致。

7. **交叉校验** — 将独立参数估算与 model card 的官方数值进行对比；解释差异（注明 MTP 层、embedding 共享、融合投影、lm_head 计法等）。
