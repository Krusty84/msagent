...
- **训推代码与资料同步更新**：训练/推理框架适配时，除代码实现需完整适配外，对应的使用资料、接入说明和操作方法也必须同步更新，确保实际使用方式与代码行为一致
- **链路打通验证**：适配完成后，必须确认 profiler 流程已打通，需验证从外部参数传递、配置解析、封装层调用到核心采集代码执行的整条链路均生效
- **强化学习分阶段解耦控制**：在强化学习场景中，profiler 控制必须按阶段解耦，不同阶段只能控制各自对应的采集流程；例如 rollout 阶段的 profiler 仅控制 rollout，actor 阶段的 profiler 仅控制 actor，避免跨阶段串扰
