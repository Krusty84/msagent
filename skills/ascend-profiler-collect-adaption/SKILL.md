...
- **强化学习分阶段解耦控制**：在强化学习场景中，性能分析器（Profiler）的控制需遵循分阶段解耦原则。各阶段仅能管控其所属的采集流程，例如 Rollout 阶段的 Profiler 仅作用于该阶段的数据采集。同时，各阶段的 Profiler 参数必须内置于对应阶段的配置项中，以此杜绝跨阶段串扰，确保采集逻辑的独立性与准确性
