...
- 若仓上已存在 NPU 训练脚本，无论是 `sh` 还是 `py` 形式，都必须在对应阶段的脚本或参数配置中补充 profiler 配置。
- 各阶段 profiler 配置默认关闭，需由用户按需开启。
- 各阶段 profiler 参数必须完整呈现，并显式给出默认值。
- actor 阶段的 profiler 配置必须写入 actor 的参数配置中，rollout 阶段的 profiler 配置必须写入 rollout 的参数配置中，其他阶段同理。
- 禁止单独创建统一的 profiler 参数数组；各阶段 profiler 参数必须分别在所属阶段配置中独立呈现，避免跨阶段串扰。
...
- 训练侧 profiler 在 offload/sleep 中被提前终止，RL 训练 actor 在 rollout 间会 offload/sleep 释放显存。若 sleep() 中无条件 prof.stop()，会中断未走完的 schedule（如 warmup 刚结束、active 尚未触发），导致无输出。 正确做法：sleep() 仅清理非 NPU profiler，NPU profiler 由 schedule 耗尽条件控制；且 schedule 耗尽后应延迟一个 rollout 再 stop()，避免 `on_trace_ready` 落盘被打断。
