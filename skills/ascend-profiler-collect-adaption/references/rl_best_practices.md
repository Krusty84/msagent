...
需要特别注意的是：
- 强化学习场景中的 profiler 控制必须按阶段解耦，不同阶段只能控制各自对应的采集流程。例如 rollout 阶段的 profiler 仅控制 rollout，actor 阶段的 profiler 仅控制 actor；同时，Profiler 参数**内置于**所属阶段的参数数组中（如 `ROLLOUT_ARGS`），不单独创建 profiler 数组，避免跨阶段串扰。
