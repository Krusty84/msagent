...
- 本 Skill 默认使用 `torch_npu.profiler.tensorboard_trace_handler("{OUTPUT_DIR}")`
- 在当前 `torch_npu.profiler` 场景下，验证口径以 `ASCEND_PROFILER_OUTPUT/` 下的解析产物为准
- 不要按通用 PyTorch Profiler 的 `.pt.json`、`trace_*.json`、`memory_*.json` 目录结构做成功判定
