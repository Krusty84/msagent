...
description: Analyze Ascend NPU schedule and Host Bound profiling issues in PyTorch or MindStudio profiler data. Use when Codex needs to diagnose device Free time, framework/operator dispatch latency, PYTORCH_API/CANN_API launch gaps, aclrtSynchronizeStream stalls, task queue behavior, CPU scheduling interference, GC/lock pauses, CPU affinity, ACLNN_CACHE_LIMIT, or schedule-side optimization actions using msprof-mcp and msprof-analyze.
---
