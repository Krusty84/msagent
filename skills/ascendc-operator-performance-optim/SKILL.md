...
**修改代码前 MUST 加载 reference/code-gen下文件**，
确保对 AscendC API、数据搬运、同步控制等有准确理解。

按需加载以下 reference（位于 `ascendc-operator-performance-optim/references/code-gen`）：
...
### 6.1 安全操作规范
- 修改源码前必须备份（`.bak`）
- 每次修改后必须验证功能正确性（检查 "precision pass"）
