...
3. **禁止孤立分析单卡**：在集群场景下，严禁仅分析单个 Rank 数据而不进行多卡对比。命令行失败后不能继续读取各个rank的数据。
...
2. **执行分析命令**：
   - 执行命令：`msprof-analyze -m <mode> -d <cluster_data> [-o <output_path>] [--force] --quiet`

3. **读取输出结果**：
   - 根据命令运行返回的结果，读取生成的文件。
   - 如果返回信息有error，或者json里面message为空，不用往下运行。提示让用户自己执行命令（去掉--quiet参数），查看日志定位原因。
...
```bash
# 【集群综合分析】
msprof-analyze -m <mode> -d <cluster_data_path> [-o <output_path>] [--force] --quiet
```
