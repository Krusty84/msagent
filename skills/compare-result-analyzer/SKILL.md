...
- 总结报告将分析报告和验证报告的核心结论整合为精简的两章结构：
  1. **结论与建议**：待排查节点（嫌疑候选，按优先级高/中/低分组，**同一优先级内按起始行号升序**）、已排除节点、后续行动建议
  2. **详细发现**：可疑候选列表与验证结果的交叉对照表（Module 算子无法自动验证，原因并入验证结论列）
- **结论性质约束**：总结报告 SHALL NOT 出现「已确认根因」小节——本 skill 仅提供分析与建议，不产出"已确认根因"，所有候选均为嫌疑（按高/中/低优先级分组，**同一优先级内按起始行号升序**）；最终根因需人工结合报告、代码与补充验证实锤。唯一可确认的是「已排除」——单算子验证通过可确认某算子实现一致、排除嫌疑
