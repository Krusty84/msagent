...
description: "分析 msProbe compare 精度比对结果文件（CSV/XLSX），定位整网精度问题。支持统计量模式，自动检测数据模式、传播跳变分析，支持多轮迭代分析上下文输入，输出结构化 Markdown 报告。使用自适应阈值级联检测（序列变点 → 锚定回溯 → Delta-NRE 离群 → 分布间隙 → 统计兜底）自动确定 NRE 阈值。"
...
- 总结报告将分析报告和验证报告的核心结论整合为精简的两章结构，头部一行「总判断」开篇：
  1. **详细发现**：可疑候选列表与验证结果的交叉对照表（Module 算子无法自动验证，原因并入验证结论列）
  2. **结论与建议**：待排查节点（嫌疑候选，按优先级）、已排除节点、后续行动建议
- **结论性质约束**：总结报告 SHALL NOT 出现「已确认根因」小节——本 skill 仅提供分析与建议，不产出"已确认根因"，所有候选均为嫌疑（按高/中/低优先级排序）；最终根因需人工结合报告、代码与补充验证实锤。唯一可确认的是「已排除」——单算子验证通过可确认某算子实现一致、排除嫌疑
