#!/usr/bin/env python3
"""
分析msprobe L1级别dump数据的比对结果文件（xlsx或csv），寻找首个输入一致输出不一致的API。

当API级分析找不到结果时，自动检测"状态跳变边界"：
  - 定位最后一个完全正常的API（输入匹配+输出匹配）
  - 定位第一个输入不匹配的API
  - 两者之间的区域即为可疑的漏采位置

用法:
    python3 find_first_diff_api_L1.py <path>

支持:
    - xlsx文件: 直接解析
    - csv文件: 直接解析
    - 目录: 自动查找目录中所有 xlsx/csv 文件，按rank排序分析

例如:
    python3 find_first_diff_api_L1.py out_llama2_L1/compare_result_rank0_20260522163310.xlsx
    python3 find_first_diff_api_L1.py out_llama2_L1_csv/
"""
...
def analyze_file(filepath, excluded_apis=None):
    ...
    api_status_list = []  # [(api_name, input_match, output_match, input_keys, output_diffs)]
    current_api = None
    current_inputs = []
    current_outputs = []
    ...
    for row in rows:
        ...
        if current_api is not None and api_name != current_api:
            status = check_api(current_api, current_inputs, current_outputs, rules)
            api_status_list.append((current_api, status))
            current_inputs = []
            current_outputs = []
    ...
    for api_name, status in api_status_list:
        ...
        if not status['output_match']:
            return {
                ...
                'found': True,
                'api': api_name,
                'input_keys': status['input_keys'],
                'output_diffs': status['output_diffs'],
            }
    ...
    first_bad_api = None
    first_bad_idx = -1
    prev_output_error = False  # 标记上一组是否有output error

    for i, (api_name, status) in enumerate(api_status_list):
        if is_api_excluded(api_name, excluded_apis):
            continue
        if status['input_match'] and status['output_match']:
            last_good_api = api_name
            last_good_idx = i
            prev_output_error = False
        elif status['input_match'] and not status['output_match']:
            # 这种情况应该被上面的查找捕获，这里作为防御
            prev_output_error = True
        elif not status['input_match']:
            first_bad_api = api_name
            first_bad_idx = i
            break
    ...
    if last_good_api is not None or first_bad_api is not None:
        ...
        if last_good_idx >= 0 and first_bad_idx >= 0 and first_bad_idx - last_good_idx > 1:
            between_apis = []
            for j in range(last_good_idx + 1, first_bad_idx):
                between_apis.append(api_status_list[j][0])
            transition_info['between_apis'] = between_apis
...
def analyze_path(path, excluded_apis=None):
    ...
    for r in all_results:
        ...
        if r['found']:
            ...
            if r.get('transition'):
                t = r['transition']
                lines = [r.get('reason', '未找到首个输入一致输出不一致的API')]
                if t.get('last_good_api'):
                    lines.append(f"")
                    lines.append(f"最后一个完全正常的API (输入匹配+输出匹配):")
                    lines.append(f"  {t['last_good_api']}")
                if t.get('first_bad_api'):
                    lines.append(f"第一个输入不匹配的API:")
                    lines.append(f"  {t['first_bad_api']}")
                if t.get('between_apis'):
                    lines.append(f"两者之间的API (可能有异常或被漏采):")
                ...
                lines.append(f"  3. 通信API (all_reduce等) 传递了其他rank的diff结果")
                kv_rows.append(('API分析依据', '\n'.join(lines)))
            else:
...
def main():
    if len(sys.argv) < 2:
        ...
        print("示例:")
        print("  python3 find_first_diff_api_L1.py out_llama2_L1/compare_result_rank0_20260522163310.xlsx")
        print("  python3 find_first_diff_api_L1.py out_llama2_L1_csv/")
        sys.exit(1)

    excluded_apis = parse_excluded_apis()
    analyze_path(sys.argv[1], excluded_apis)
