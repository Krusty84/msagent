#!/usr/bin/env python3
"""
分析msprobe mix级别dump数据的比对db文件，寻找首个输入一致输出不一致的API。

支持多卡场景: 同一step的所有rank并行分析，表格形式输出结果。
同时分析module层级: 展示首个问题module的父子层级链路，帮助定位漏采API。

用法:
    python3 find_first_diff_api_mix.py <db_path>

例如:
    python3 find_first_diff_api_mix.py out_mix_1_2/compare_20260521162624.vis.db
"""
...
from utils import build_excluded_rules, is_key_excluded, print_kv_table, parse_excluded_apis, is_api_excluded
...
def extract_md5_values(data_json, node_name=None, excluded_rules=None):
    ...
    for key, value in data.items():
        if isinstance(value, dict) and 'md5' in value:
            if node_name and excluded_rules:
                # 从key中解析io_type和index，如 "NPU.xxx.input.0" → ('input', '0')
                excluded = False
...
def build_hierarchy(cursor, node_id, data_source, step, rank):
    """
    通过追踪up_node构建从root到指定节点的层级路径。
    返回 (路径列表, 子节点列表)
    路径格式: [(node_name, node_type), ...]
    """
    path = []
    current_id = node_id
    max_depth = 50  # 防止死循环
    ...
    for _ in range(max_depth):
        ...
        if not up_node or up_node == 'None':
            break
        # up_node 存的是 node_name，在当前 data_source/step/rank 下查找
        cursor.execute(
...
def analyze_rank(db_path, step, rank, excluded_apis=None):
    ...
    cursor.execute(
        """SELECT id, node_name, node_order, node_type, input_data, output_data
           FROM tb_nodes
           WHERE data_source='NPU'
             AND precision_index=1
             AND step=?
             AND rank=?
           ORDER BY node_order""",
        (step, rank)
    ...
    if npu_error_nodes:
        for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_error_nodes:
            ...
            npu_input_md5 = extract_md5_values(npu_input, node_name, excluded_rules)
            bench_input_md5 = extract_md5_values(bench_input, node_name, excluded_rules)
            npu_output_md5 = extract_md5_values(npu_output, node_name, excluded_rules)
            bench_output_md5 = extract_md5_values(bench_output, node_name, excluded_rules)

            input_match, _, all_input_keys = compare_md5(npu_input_md5, bench_input_md5)
            output_match, output_diffs, _ = compare_md5(npu_output_md5, bench_output_md5)
            ...
            if not output_match:
                api_result = {
                    'found_api': True,
                    'node_name': node_name,
                    'node_type': 'API' if node_type == '1' else ('Module' if node_type == '0' else f'Type_{node_type}'),
                    'node_order': node_order,
                    'npu_id': npu_id,
                    'bench_id': bench_id,
                    'input_keys': all_input_keys,
                    'output_diffs': output_diffs,
                    'npu_input_md5': npu_input_md5,
                    'bench_input_md5': bench_input_md5,
                    'npu_output_md5': npu_output_md5,
                    'bench_output_md5': bench_output_md5,
                    'hierarchy': build_hierarchy(cursor, npu_id, 'NPU', step, rank),
                }
                break

    # ── Module级分析 ──
    # 如果API级已找到结果，不需要再分析module
    module_result = None
    ...
    for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_module_nodes:
        ...
        # Module可能没有明确的input_data，跳过无数据的
        if not npu_input_md5 and not npu_output_md5:
            continue

        input_match, _, _ = compare_md5(npu_input_md5, bench_input_md5)
        output_match, output_diffs, _ = compare_md5(npu_output_md5, bench_output_md5)
    ...
    result = {
        'step': step,
        'rank': rank,
        'found': api_result is not None,
    }

    if api_result:
        result.update(api_result)
    else:
        result['reason'] = '未找到输入一致输出不一致的API'
...
def analyze_db(db_path, excluded_apis=None):
    ...
    for step in steps:
        ...
        with ThreadPoolExecutor(max_workers=len(ranks_in_step)) as executor:
            future_to_rank = {
                executor.submit(analyze_rank, db_path, step, rank, excluded_apis): rank
                for rank in ranks_in_step
            }
        ...
        for r in step_results:
            ...
            if r['found']:
                kv_rows.append(('首个问题API', r['node_name']))

                # API所在Module层级
                hier_lines = []
                if r.get('hierarchy'):
                    dl = 0
                    for node in r['hierarchy']:
                        ...
                        prefix = '→ ' if dl > 0 else ''
                        hier_lines.append(f"{'  ' * dl}{prefix}{name} [{tl}]")
                        dl += 1
                kv_rows.append(('API所在Module层级', '\n'.join(hier_lines) if hier_lines else '-'))

                # API分析依据
                analysis = ['Input MD5 (全部一致):']
                for k in r.get('input_keys', []):
                    analysis.append(f"  {k}")
                analysis.append('Output MD5 (不一致):')
                for d in r.get('output_diffs', []):
                    analysis.append(f"  {d}")
                kv_rows.append(('API分析依据', '\n'.join(analysis)))

                kv_rows.append(('首个问题Module', '已找到首个问题API，不分析'))
                ...
                if ma:
                    ...
                    m_analysis = ['Module Output MD5 (不一致):']
                    for d in ma.get('output_diffs', []):
                        m_analysis.append(f"  {d}")
                    kv_rows.append(('Module分析依据', '\n'.join(m_analysis)))
                else:
                    kv_rows.append(('首个问题Module', '无'))
                    kv_rows.append(('Module分析依据', '未找到首个输入一致输出不一致的Module'))

            # 打印双列表
            print_kv_table(r['rank'], kv_rows)

            # 未找到API但找到Module → 打印可能漏采信息
            if not r['found'] and r.get('module_analysis'):
                ma = r['module_analysis']
                print(f"  可能原因: 该Module内可能有被msprobe漏采的API，导致Module整体输出不一致。")
                if ma.get('hierarchy'):
                    print(f"  请检查以下层级的子节点列表，确认是否有API未被采集:")
                    dl = 0
                    for node in ma['hierarchy']:
                        nt = node.get('node_type', '')
                        if nt not in ('0', '1'):
                            continue
                        tl = {'0': 'Module', '1': 'API'}.get(nt, '?')
                        name = node.get('node_name', '?')
                        prefix = '→ ' if dl > 0 else ''
                        print(f"    {'  ' * dl}{prefix}{name} [{tl}]")
                        dl += 1

        if found_in_step := [r for r in step_results if r['found']]:
...
def main():
    if len(sys.argv) < 2:
        print("用法: python3 find_first_diff_api_mix.py <db_path>")
        print("示例: python3 find_first_diff_api_mix.py out_mix_1_2/compare_20260521162624.vis.db")
        sys.exit(1)

    db_path = sys.argv[1]
    excluded_apis = parse_excluded_apis()
    analyze_db(db_path, excluded_apis)
