...
def analyze_rank(db_path, step, rank, excluded_apis=None):
    ...
    if npu_error_nodes:
        for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_error_nodes:
            ...
            m = lambda x: extract_md5_values(x, node_name, excluded_rules)  # noqa: E731
            input_match, _, all_input_keys = compare_md5(m(npu_input), m(bench_input))
            output_match, output_diffs, _ = compare_md5(m(npu_output), m(bench_output))
            if not input_match:
                continue
            if not output_match:
    ...
    for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_module_nodes:
        ...
        bench_input, bench_output = bench_row
        m = lambda x: extract_md5_values(x, node_name, excluded_rules)  # noqa: E731
        if not m(npu_input) and not m(npu_output):
            continue
        input_match, _, _ = compare_md5(m(npu_input), m(bench_input))
        output_match, output_diffs, _ = compare_md5(m(npu_output), m(bench_output))
        if not input_match:
            continue
        if not output_match:
