...
def analyze_rank(db_path, step, rank, excluded_apis=None):
    ...
    if api_result is not None:
        conn.close()
        return {'step': step, 'rank': rank, 'found': True, **api_result}
    cursor.execute(
        "SELECT id, node_name, node_order, node_type, input_data, output_data"
...
def main():
    if len(sys.argv) < 2:
        ...
        print("示例: python3 find_first_diff_api_mix.py out_mix/compare_20260521162624.vis.db")
        sys.exit(1)
    analyze_db(sys.argv[1], parse_excluded_apis())
