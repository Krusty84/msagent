...
def analyze_rank(db_path, step, rank, excluded_apis=None):
    ...
    if npu_error_nodes:
        for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_error_nodes:
            ...
            if not output_match:
                ...
                api_result = {
                    'node_name': node_name, 'node_type': type_label, 'node_order': node_order,
    ...
    for npu_id, node_name, node_order, node_type, npu_input, npu_output in npu_module_nodes:
        ...
        if not output_match:
            hierarchy = build_hierarchy(cursor, npu_id, 'NPU', step, rank)
            module_result = {
                'module_name': node_name, 'node_order': node_order,
                'output_diffs': output_diffs, 'hierarchy': hierarchy,
            }
            break
    ...
    for step in steps:
        ...
        for r in step_results:
            ...
            if r['found']:
                ...
                hier_lines = format_hierarchy(r.get('hierarchy', []))
