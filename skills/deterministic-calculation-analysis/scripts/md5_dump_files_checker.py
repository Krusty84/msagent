...
def validate_csv_xlsx(root_path):
    """校验csv/xlsx文件头是否包含NPU MD5和BENCH MD5列。"""
    ...
    try:
        if first.endswith('.csv'):
            ...
            fieldnames = [str(c.value) if c.value is not None else '' for c in next(wb.active.iter_rows(min_row=1, max_row=1))]
            wb.close()
