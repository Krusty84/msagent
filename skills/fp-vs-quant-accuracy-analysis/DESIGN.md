...
| **异常激活输出策略** | compare 结果输出**所有 COS < 0.95 的异常激活**列表（非首个偏差模块），按 md5 状态区分异常源头与传播点（决策 3.14） |
...
flowchart TD
    ...
    F -- "是" --> G["输出全量异常激活列表<br/>COS + md5 状态（源头 / 传播点）"]
...
**决策**：compare 结果输出**所有 COS 相似度 < 0.95 的激活**列表（按执行顺序，不限于首个），每个激活标注 md5 状态以区分**异常源头**（输入 md5 一致且输出不一致，异常在本模块产生）与**异常传播点**（输入输出均不一致，前层异常所致）。阈值默认 0.95（95% 相似度）可配置。无任何 COS < 0.95 的激活 → 无单点异常，输出"累积误差"结论。输出字段与示例见 references/subagent_io.md（`anomaly_modules` 数组）。
...
- **compare 配置**：`msprobe compare -tp <dump_quant> -gp <dump_fp> -c cos,md5,max_diff` + TensorBoard 可视化
...
| 结论 | 异常激活列表（module/COS/md5 状态）、阈值、TensorBoard 路径、报告路径 |
