...
| **输入收敛** | 用户输入收敛为 6 项：量化配置 `{model_type}_best_practice.yaml` 由 agent 在量化权重路径下**自主获取**（决策 3.13）；量化命令由 agent 以该 yaml 为 `--config-path` + `--debug` 自然语言拉起复现，用户无需提供 |
| **异常激活输出策略** | compare 结果输出**所有 COS < 0.95 的异常激活**列表（非首个偏差模块，阈值可配置）（决策 3.14） |
| **失败路径与错误处理** | 各环节失败统一输出"失败环节 + error.code + 证据摘要 + 建议下一步"，执行层 5 次未解决上报编排层（决策 3.12） |
