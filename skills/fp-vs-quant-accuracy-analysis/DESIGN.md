...
Agent 当前仅支持 LLM 调优（调整 Practice YAML 提升量化精度），但当出现**算子异常**（量化实现与浮点行为不一致）或**框架组图异常**（量化后计算图结构异常）时，调优无法继续，需要一种精度异常定位能力：对比浮点权重推理与量化权重推理的中间激活值，找到出现显著偏差的模块。

#### 人工定位的本来流程（基线）

在引入本 Agent 之前，精度异常定位完全依赖人工，流程如下（按阶段归纳）：

| 阶段 | 人工操作步骤 | 说明 |
|------|-------------|------|
| **A. 环境与产物准备** | 1. 前提条件检查 | 浮点模型真实存在、npu 环境、vllm 环境 |
| | 2. 安装工具 | 安装 msprobe、msmodelslim |
| | 3. 量化模型 | 产出量化权重（含旋转/抑制中间量） |
| **B. dump 采集** | 4. vllm 拉起 + 单 curl 触发 dump | 通过加入 probe dump 参数的 vllm 分别拉起浮点模型和量化模型；根据异常的 badcase 进行单 curl；只需一张卡的一个 step，跑一轮即可 |
| **C. 逆变换与比对** | 5. 逆变换推导 | 获取 dump 后，根据**模型适配器代码**和 **transformers 代码**，对旋转和离群值抑制的激活进行逆转 |
| | 6. compare 比对 | 逆转之后执行 compare（msprobe 在 compare 时做 tensor 后处理） |
| **D. 分析结论** | 7. 分析 compare 结果 | 人工判断异常激活与归因 |

**人工流程的痛点（本次 Agent 要解决的）**：

1. **知识门槛高**：步骤 5 需要人工读懂模型适配器代码（`get_rotate_map` 的旋转作用范围）与 transformers 模型结构代码，推导哪些激活在旋转空间、如何逆转——这是最耗时且最容易出错的环节
2. **手工操作繁重**：7 步跨多个工具（msmodelslim / vllm / msprobe / TensorBoard），环境准备、命令构造、产物管理全靠人工
3. **方案识别靠经验**：是否需要用 `--debug` 复现量化、旋转/抑制中间量从哪里取（`optional/` vs `debug_info/`），依赖对量化方案的判断
4. **结论分析靠人**：compare 结果需要人工解读，异常可能多模块同时出现，归因容易片面

#### 本次 Agent 的目的

**把上述人工流程端到端自动化**：用户只需提供模型路径、vllm 命令、模型侧文件等业务信息，Agent 自动完成——量化方案识别与复现决策（步骤 0）、dump 采集（步骤 1-3）、逆变换推导（阅读适配器 + transformers 代码生成 rotate_map 并转换中间量）、compare 比对与异常激活汇总（步骤 4-6），最终输出带证据的异常激活列表。

### 0.2 本次设计要点（v0.5.0）

| # | 修改点 | 修改内容 | 原因 |
|---|--------|---------|------|
| 1 | **与调优并列的端到端流程** | 由"调优互斥路径"重构为与 `quantization-accuracy-tuning-orchestrator` **并列**的端到端 workflow skill，Quantizer 主会话按意图路由加载，一次任务只走一条 | 人工定位本身是一条完整流程（见 0.1），与调优是并列关系而非其附属；互斥只存在于入口路由 |
| 2 | **执行层 subagent 化** | 对齐 orchestrator 两层架构：编排层（主会话）收集输入/委派/汇总；执行层 3 个 subagent 承载——`quant-tuning-accuracy-quantizer`（步骤0）、`quant-tuning-accuracy-collector`（步骤1-3）、`quant-tuning-accuracy-comparator`（步骤4-6） | 拉服务采集 dump、复现量化、compare 比对是重型步骤，需要上下文隔离与失败重试隔离；与 orchestrator 架构对齐 |
| 3 | **输入收敛为 6 项** | 量化配置 `{model_type}_best_practice.yaml` 由 agent 在量化权重路径下**自主获取**（决策 3.13）；量化命令由 agent 以该 yaml 为 `--config-path` + `--debug` 自然语言拉起复现 | yaml 是量化产物的标准组成文件，用户重复提供属冗余输入；人工步骤 3"量化模型"本就不需要用户给命令 |
| 4 | **rotate_map 双文件输入** | 生成 rotate_map.json 需要 msmodelslim 模型适配器文件 + transformers 模型结构文件两个输入（决策 3.8） | 对应人工步骤 5：逆变换推导必须同时依据适配器代码（旋转作用范围）与结构代码（模块层级与激活流向） |
| 5 | **异常激活输出策略** | compare 输出**所有 COS < 0.95 的异常激活**列表（非首个偏差模块，阈值可配置）（决策 3.14） | 异常可能多模块同时出现（系统性偏差、链路传播），只报首个丢失全貌（对应人工步骤 7 的归因片面问题） |
| 6 | **失败路径与错误处理** | 各环节失败统一输出"失败环节 + error.code + 证据摘要 + 建议下一步"，执行层 5 次未解决上报编排层（决策 3.12） | 流程跨多个重型环节，失败必须可定位、可上报、可询问，避免静默重试或编造结论 |
| 7 | **SKILL.md 编排化** | 重写为 workflow 编排风格：整体设定（编排者角色）+ 执行委派 + 用户输入确认 → 步骤 0-6 → 结果输出；Quantizer 硬性规则"禁止读代码仓"增加豁免（阅读用户明确提供的适配器/结构文件） | 与 orchestrator 文档形态一致；人工步骤 5 阅读适配器/transformers 代码在 agent 化后仍需保留，需与读代码仓禁令协调 |
...
1. 用户通过 msagent CLI 启动 Quantizer，用自然语言提出定位需求（如"量化后精度异常，帮我定位到具体模块"）
2. Quantizer 按意图路由到本 Skill（`get_skill(name="fp-vs-quant-accuracy-analysis")`）
3. 编排层（主会话）收集用户输入并**回显确认**——用户只需提供业务信息，无需提供技术命令
4. 编排层依次委派 3 个执行 subagent 完成步骤 0-6
5. 编排层汇总输出定位报告（异常激活列表 + 证据 + 可视化入口）
...
| 3 | msmodelslim 模型适配器文件路径 | QuaRot 必需 | `get_rotate_map` 定义旋转作用范围（对应人工步骤 5） |
| 4 | 模型结构文件路径（transformers `modeling_*.py`） | QuaRot 必需 | 模型层级结构与激活流向（对应人工步骤 5） |
| 5 | 保存路径（输出目录） | ✅ | 中间产物与结果；用户指定优先，缺省 agent 建议 |
| 6 | 推理请求内容 | ⬜ | 触发 dump 的 prompt，默认 `"Hello" + max_tokens=1`；异常仅在特定输入复现时可提供该 badcase（对应人工步骤 4"根据异常 badcase 单 curl"） |

**无需用户提供**（agent 自主获取/推断）：量化配置 `{model_type}_best_practice.yaml`（量化权重路径下自主获取）、量化命令（以 yaml 为 `--config-path` + `--debug` 自然语言拉起）。

### 场景用例

| 用例 | 触发条件 | 说明 |
|------|---------|------|
| 量化精度异常定位（主场景） | 量化推理出现精度异常，需定位到具体 module | 对比浮点 vs 量化 dump，筛出所有 COS 不达标激活并定位异常模块 |
| 调优无法继续时定位 | 算子/框架组图异常导致调优流程无法继续 | 先定位异常模块，为修复提供方向 |
| 量化方案排查 | W8A8 + QuaRot + SmoothQuant 等方案 | 支持逆变换后对齐数值空间再比对 |

### 上下游依赖（分开表述）

**上游依赖**（本 Skill 使用的外部输入，按类别）：

| 类别 | 依赖 | 说明 |
|------|------|------|
| 环境 | npu 硬件 | 推理与量化运行环境（A2/A3/A5） |
| 环境 | vllm / vllm-ascend | 服务化部署与 `dump_config_path` 触发（决策 3.9） |
| 工具 | msprobe（mindstudio-probe） | dump 采集、compare 比对、tensor 后处理（决策 3.1/3.7/3.10） |
| 工具 | msmodelslim | 量化（复现中间量）；`{model_type}_best_practice.yaml` 算法判断依据（决策 3.11） |
| 模型 | 浮点模型 | 前提条件：真实存在（人工步骤 1） |
| 模型 | 量化模型产物 | 含 yaml、旋转矩阵（`optional/`）、抑制 scales（`debug_info/`）；缺失时通过复现补齐 |
| 模型 | 适配器文件 + 结构文件 | rotate_map 生成输入（决策 3.8，对应人工步骤 5） |

**下游依赖**（本 Skill 的输出被谁使用）：

| 类别 | 下游 | 说明 |
|------|------|------|
| 可视化 | TensorBoard | compare 结果可视化入口 |
| 用户 | 定位报告 | 异常激活列表 + 证据，供用户决策修复方向 |
| 联动 | msmodelslim PR #789 | 前置功能依赖（见「风险与外部依赖」章节），未合入前 Fusion 路径不可用 |

### Agent 角色理解（端到端精度异常定位 Agent 是什么）

本 Agent 是**人工定位流程的自动化替代者**，不是调优的附属工具：

| 人工环节 | Agent 对应实现 |
|---------|---------------|
| 步骤 1-3：环境检查、装工具、量化模型 | 步骤 0：方案识别（读 yaml 判断算法）→ 需要时自动 `--debug` 复现量化 |
| 步骤 4：vllm 拉起 + 单 curl 触发 dump | 步骤 1-3：自动生成 probe 配置、追加 vllm 参数、单请求触发 dump（1 step + 1 rank） |
| 步骤 5：读适配器 + transformers 代码推导逆变换 | 步骤 2：自动阅读两个模型侧文件生成 rotate_map.json、转换旋转矩阵与抑制因子 |
| 步骤 6：逆转后 compare | 步骤 4-5：自动生成并部署 tensor 后处理配置、执行 compare（compare 时自动逆变换） |
| 步骤 7：分析 compare 结果 | 步骤 6：自动汇总所有 COS < 0.95 的异常激活 |

**角色定位**：编排者（收集输入、回显确认、按序委派、汇总输出）+ 知识载体（量化方案识别、逆变换数学、msprobe/vllm 集成细节）。**边界**：不做调优（与 orchestrator 并列路由）、不改业务/框架源码、结论必须基于真实 dump 数据。

### 成功标准（验收条件）

验收用例（MiniMax-M3 W8A8，A2/A3 硬件）：
...
flowchart TB
    ...
    subgraph EXEC["执行层 · Subagent（msagent-io v1 协议）"]
        ...
        subgraph LOC["定位执行（3 个，依次委派）"]
            ...
            B3["quant-tuning-accuracy-comparator<br/>步骤4-6 后处理 / compare / 汇总异常激活"]
        end
    end
...
```mermaid
flowchart TD
    A["① 收集用户输入<br/>6 项参数（模型路径 / vllm 命令 / 适配器文件 /<br/>模型结构文件 / 保存路径 / 推理请求）"] --> B["② 参数回显确认（用户认可）"]
    B --> C{"量化配置算法判断<br/>自主获取 {model_type}_best_practice.yaml<br/>含旋转/抑制算法？"}
    C -- "需要复现" --> D1["委派 quant-tuning-accuracy-quantizer<br/>步骤0 调试模式复现量化<br/>--config-path yaml --debug"]
    C -- "无需复现" --> D2
    ...
    D3 --> E["③ 结果输出<br/>异常激活列表 + 证据 + 可视化 + 审计日志"]
    E --> F{"存在 COS < 0.95 的异常激活？"}
    F -- "是" --> G["输出全量异常激活列表<br/>module + COS"]
    F -- "否（累积误差）" --> H["建议：放宽阈值 / 补采 module /<br/>扩展 step-rank"]
```

### 执行阶段说明（分阶段）

流程按**人工定位的四个阶段**组织，每阶段说明各步骤做什么与注意点：

#### 阶段 A：环境与产物准备（步骤 0）

**做什么**：判断量化方案是否含旋转/抑制算法、是否需要复现中间量；需要时以 `{model_type}_best_practice.yaml` 为 `--config-path` + `--debug` 复现量化。

**注意点**：

- `{model_type}_best_practice.yaml` 在量化权重路径下**自主获取**，无需用户提供；该文件即用户量化配置本身，复现与产物同源
- 判断依据是 **算法类型集合 + 产物现状**（非产物完整性）：旋转矩阵是常规输出（`optional/` 已有则免复现）；抑制 scales 是 debug 中间量（无 `debug_info/` 必须复现）——这是最容易判断错的地方
- 复现成功的判定：exit code 0 + 产物校验通过（含算法对应的 `debug_info/` 或 `optional/`）
- 不适用 `online_quarot`（在线旋转，旋转矩阵分散保存）

#### 阶段 B：dump 采集（步骤 1-3）

**做什么**：生成两侧 probe.json（步骤 1）→ 逆变换准备：rotate_map.json + 旋转矩阵/抑制因子格式转换（步骤 2）→ 在用户 vllm 命令上追加 `--enforce-eager` 与 `dump_config_path`，两侧各拉一次服务、各发一次请求触发 dump（步骤 3）。

**注意点**：

- **默认 1 step + 1 rank**：一个 token 的激活足够定位，单卡采集（对应人工步骤 4"只要一张卡上的一个 step，跑一轮即可"）
- **`--enforce-eager` 必须**：msprobe dump 只在 eager 模式生效
- **推理请求**：默认 `"Hello" + max_tokens=1` 即可；异常仅在特定输入复现时用该 badcase 触发（对应人工步骤 4"根据异常的 badcase 单 curl"）
- rotate_map.json 生成**必须同时阅读适配器文件与结构文件**：适配器 `get_rotate_map` 给出旋转作用范围（key 为完整模块路径），结构文件确认模块层级与激活流向（哪些中间激活在旋转空间）；只读其一会漏配或错配逆变换
- 抑制因子转换注意 NonFusion（`div.mul_scale`，扫描到 0 个说明全走 Fusion）与 Fusion（`debug_info` 提取，需 `--debug`）两条路径的区分

#### 阶段 C：逆变换与比对（步骤 4-5）

**做什么**：生成 msprobe tensor 后处理配置并部署到 `tensor_postprocess/`（步骤 4）→ `msprobe compare -c cos,max_diff` 比对 + TensorBoard 可视化（步骤 5）。

**注意点**：

- **后处理在 compare 阶段生效**（读入 tensor 后、校验前），不修改 dump 文件——配置可迭代，改配置重新 compare 即可
- 所有逆变换统一 **matmul**（逆抑制用 `diag(s)` 对角矩阵），msprobe 原生支持，无需扩展
- QuaRot 场景强烈推荐 `--dump-json`：自动推导右旋空间中间模块（含 RMSNorm）的逆变换规则，不传会漏配
- msprobe 用完整 data_name 精确匹配（不支持正则）：必须用 `inspect_dump.py` 提取实际 data_name 替换占位符
- 两侧模块名因 Wrapper 不一致时配置 `data_mapping` 映射
- compare 指标为 `cos,max_diff`（不使用 md5）

#### 阶段 D：分析与输出（步骤 6 + 结果输出）

**做什么**：筛出**所有** COS < 0.95 的异常激活（module + target + COS，按执行顺序全量列出）→ 输出报告（异常激活列表、证据、逆变换信息、TensorBoard、审计日志）。

**注意点**：

- **全量而非首个**：异常可能多模块同时出现，只报首个会丢失全貌（决策 3.14）
- 阈值默认 0.95 可配置；无任何异常激活 → "累积误差"结论（非错误），建议放宽阈值/补采/扩展 step-rank
- 结论必须附证据（dump 路径、COS、compare 结果目录），审计日志全流程记录
...
- 不存在的路径必须向用户索取，禁止无范围的全盘搜索；在用户提供的量化权重模型路径下自主查找 `{model_type}_best_practice.yaml` 除外（决策 3.13）

### 安全敏感操作

- 本 Skill 会拉起用户环境的 vllm serve 服务并发送推理请求（复用用户提供的启动命令，不新增服务）
- 结论禁止编造：所有指标必须来自真实 dump 数据并附证据（路径、shape）
...
| `quant-tuning-accuracy-comparator` | 步骤 4-6 | 后处理配置 + compare + 汇总异常激活 |
...
- 领域协议：本 Skill 参与 msagent-io 委派协议（与 orchestrator 同一协议总则 `subagent_io_protocol.md`），字段表见本 skill `references/subagent_io.md`
...
| 基础设施 | 状态 | 承载的诉求 |
|----------|------|-----------|
| msprobe（mindstudio-probe） | ✅ 无变更 | dump 采集（`task=tensor, level=L0`）；compare 比对（`cos,max_diff`）；tensor 后处理（逆变换，compare 阶段实时生效） |
...
| 维度 | 评估 | 说明 |
|------|------|------|
| 采集开销 | 默认 1 step + 1 rank | 一个 token 的激活足够筛出异常激活列表，数据量最小化 |
...
| 阶段 | 内容 | 并行性 | 验收点 |
|------|------|--------|--------|
| 1. 技术调研 | msprobe 机制验证（后处理时机、vllm 集成、dump 格式） | — | 决策 3.1-3.14 依据成立 |
...
| 5. 端到端验证 | MiniMax-M3 W8A8（A2/A3）验收用例 | — | 4 条验收标准通过 |
...
本 Skill 依赖 msmodelslim 侧的前置功能合入，未合入前对应路径不可用：

| 依赖 | 前置功能 | 影响路径 | 未合入前行为 |
|------|---------|---------|-------------|
| msmodelslim PR [#789](https://gitcode.com/Ascend/msmodelslim/pull/789) | 修复 `QuaRotProcessor._record_debug_info` 去重（量化开 `--debug` 卡死 4-9 小时的问题） | 步骤 0 复现量化（`--debug`）与 Fusion 逆抑制路径 | 量化开 `--debug` 卡死，`debug_info.safetensors` 无法落盘；含旋转 + 抑制算法的量化方案（如 MiniMax-M3 QuaRot + iter_smooth）无法复现获取中间量，Fusion 逆抑制无法配置 |

联动设计依据：本仓库 [docs/design_quarot_debug_dedup.md](../../../docs/design_quarot_debug_dedup.md)（该 PR 的根因分析、修复方案与下游兼容性说明）。

### 其他外部依赖（常规，非阻塞）

| 依赖 | 能力 | 说明 |
|------|------|------|
| msprobe（mindstudio-probe） | dump / compare（`-c cos,max_diff`）/ tensor 后处理 | 依赖版本提供 `tensor_postprocess` 机制（决策 3.1/3.7/3.10） |
| vllm-ascend | `--additional-config` 解析 `dump_config_path` | 依赖版本支持该字段（决策 3.9） |
...
|------|------|---------|---------|---------|
| fp-vs-quant-accuracy-analysis/SKILL.md | Skill 组件 | 🔧 | 端到端定位编排 | workflow 风格重写（metadata/整体设定/执行委派/输入确认/结果输出）；步骤 0 改为基于 `{model_type}_best_practice.yaml` 算法判断复现；步骤 2.1 rotate_map 输入拆为适配器文件 + 模型结构文件；步骤 6 改为汇总所有 COS < 0.95 的异常激活；新增失败路径与错误处理章节 |
...
| quant-tuning-accuracy-comparator.yml/.md | 配置/Prompt | 🆕 | 执行层-步骤4-6 | 后处理配置 + compare + 汇总所有 COS < 0.95 的异常激活 |
...
1. **技术调研**（阶段 1）：msprobe/vllm 机制验证 → 验收：决策 3.1-3.14
2. **脚本开发**（阶段 2，可并行）：7 个脚本 → 验收：单脚本可执行
3. **Skill 编排**（阶段 3）：SKILL.md + DESIGN.md → 验收：全流程可走通
4. **Agent 集成**（阶段 4）：Quantizer 路由与约束 → 验收：意图路由正确
5. **端到端验证**（阶段 5）：MiniMax-M3 W8A8（A2/A3）→ 验收：4 条验收标准

---

## 附录 A：核心技术决策（编号保留，供 SKILL.md 引用）
...
初版硬编码 `--model-type minimax_m3` 与内置旋转规则。**改为 `--rotate-map` 参数以 JSON 文件传入**（`right_input`/`right_output`/`left_output` 分类），支持任意模型。

rotate_map.json 由 agent 依据两个模型侧输入文件生成：**msmodelslim 模型适配器文件**（`get_rotate_map` 定义旋转作用范围，key 为完整模块路径）+ **transformers 模型结构文件**（确认模块层级与激活流向）。输入要求与生成规则见 SKILL.md 步骤 2.1。
...
**决策**：在量化权重模型路径下**自主获取** `{model_type}_best_practice.yaml`（该文件即用户量化配置本身），按 `spec.process` 的 **processor 算法类型集合**（旋转类 `quarot`/`adapt_rotation`，抑制类 `smooth_quant`/`iter_smooth`/`flex_smooth_quant`/`flex_awq_ssz`/`awq`/`kv_smooth`/`oasq`，不支持 `online_quarot`）+ 产物现状（`optional/`、`debug_info/`）判断是否需要复现。判断表见 SKILL.md 步骤 0。

**关键区分**（两种中间量的落盘机制差异，决定判断逻辑）：
...
**维护注意事项**：算法类型集合为**静态维护清单**（维护点在 SKILL.md 步骤 0 与 quantizer subagent prompt 两处；本文档仅记录决策，不重复维护）。msmodelslim 算法持续演进，新增旋转或抑制类算法时必须同步更新，否则会漏判（把含新抑制算法的配置误判为"无需复现"）。核对来源为 msmodelslim《量化算法总览》（`docs/zh/knowledge_base/quantization_algorithms/README.md`）及 processor 注册表（`msmodelslim/processor/` 下各算法的 `Literal["type"]` 判别字段）。

### 决策 3.12：失败路径的错误输出设计

**决策**：每个执行环节失败时向用户明确输出"**失败环节 + error.code + 证据摘要 + 建议下一步**"，统一错误枚举（`VALIDATION_ERROR` / `MODEL_LOAD_ERROR` / `DUMP_ERROR` / `POSTPROCESS_ERROR` / `UNKNOWN_ERROR`，与执行 subagent 协议对齐）；执行层同一问题重试 5 次未解决上报编排层，由编排层汇总"已尝试措施"后向用户询问方案。**无 COS < 0.95 的异常激活不是错误**，输出"累积误差"结论与建议。详见 SKILL.md「失败路径与错误处理」。
...
### 决策 3.14：异常激活输出策略（全量 COS < 0.95 列表，非首个偏差模块）

**背景**：初版设计定位"首个输入一致、输出不一致的异常模块"（单一根因）。但量化异常可能**多模块同时出现**（量化实现的系统性偏差、异常沿链路传播），只报首个会丢失全貌、误导归因。

**决策**：compare 结果输出**所有 COS 相似度 < 0.95 的激活**列表（按执行顺序，不限于首个），每项含 module、target（input/output）、COS 值。阈值默认 0.95（95% 相似度）可配置。无任何 COS < 0.95 的激活 → 无单点异常，输出"累积误差"结论。输出字段与示例见 references/subagent_io.md（`anomaly_modules` 数组）。比对指标为 `cos,max_diff`，不使用 md5。
...
- **dump 配置**：`task=tensor, level=L0, step=[0], rank=[0]`（默认 1 step + 1 rank），由 `gen_msprobe_config.py` 生成
- **tensor 后处理配置**：所有规则统一 `operation: matmul` + `side` 字段；逆抑制用 `diag(s)` 对角矩阵（决策 3.7）；`--dump-json` 方案 C 自动推导右旋空间中间模块（含 RMSNorm）的逆变换规则
- **compare 配置**：`msprobe compare -tp <dump_quant> -gp <dump_fp> -c cos,max_diff` + TensorBoard 可视化

完整命令与参数见 SKILL.md 步骤 1 / 4 / 5。
...
4. 汇总所有 COS < 0.95 的异常激活（或确认无 → 累积误差）
...
| 结论 | 异常激活列表（module/COS）、阈值、TensorBoard 路径、报告路径 |
