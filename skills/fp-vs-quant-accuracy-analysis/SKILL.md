...
1. model.language_model.layers.12.mlp.down_proj.linear [output]
   Input MD5 一致(已逆抑制) | Output MD5 不一致: quant=xxx vs fp=yyy | COS=0.8765
   → 异常在本模块产生（源头）
2. model.language_model.layers.13.mlp.gate_proj [output]
   Input MD5 不一致 | Output MD5 不一致 | COS=0.9210
   → 前层异常传播所致
...
...
- 每个异常激活标注 md5 状态：**输入 md5 一致且输出不一致** → 异常在本模块产生（源头）；**输入输出均不一致** → 前层异常传播所致
...
- **所有 COS < 0.95 的异常激活列表**（含 md5 状态标注与 COS 值），或"无异常激活，偏差为累积误差"的结论
...
| 步骤 4-6 | 无 COS < 0.95 的异常激活 | **非错误**：输出"偏差为累积误差"结论 + 建议（放宽阈值 / 补采 module / 扩展 step-rank / 转调优） |
