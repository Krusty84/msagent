...
端到端精度异常定位包括**用户输入**、**环境与产物准备**、**dump 采集**、**逆变换后处理**和**结果输出**环节，适用于量化推理出现精度异常、或算子/框架组图异常导致调优无法继续的场景。流程对比浮点权重与量化权重推理的 module 级激活值 dump，量化侧先做逆变换（QuaRot 逆旋转 / SmoothQuant 逆抑制）还原到浮点数值空间，筛出所有 COS 相似度不达标的异常激活并定位异常模块。
...
| 3 | `quant-tuning-accuracy-comparator` | 步骤 4-6 | 生成后处理配置、msprobe compare 比对、汇总所有 COS < 0.95 的异常激活 |
...
#### 步骤 0 执行参数（run_quantization.py）

| 参数 | 必需 | 说明 |
|------|------|------|
| `--model-type` | ✅ | 模型类型名，从 yaml `metadata.verified_model_types` 或模型路径推断 |
| `--model-path` | ✅ | 原始浮点模型路径 |
| `--save-path` | ✅ | 量化产物保存路径（复现产物输出，如 `<workdir>/quant_model`） |
| `--config-path` | ✅ | 自主获取的 `{model_type}_best_practice.yaml`（与 `--quant-type` 互斥） |
| `--device` | ⬜ | 默认 `npu`；优先单卡（如 `npu:0`） |
| `--trust-remote-code` | ⬜ | 是否信任远程代码 |
| `--debug` | ✅ | 默认开启（`--no-debug` 关闭）；抑制 scales 必须 debug 模式 |
| `--skip-if-exists` | ⬜ | 产物已存在时跳过 |
| `--audit-log` | ✅ | 审计日志路径 |

#### 成功判定

exit code 为 0，且产物校验通过（脚本 `check_products` 检查）：

- `quant_model_weights.safetensors`（量化权重，可能分片）
- 配置含抑制类算法时：`debug_info/debug_info.safetensors`（含 `smooth_scales.*`）
- 配置含旋转类算法且需要时：`optional/quarot.safetensors`（含 `global_rotation`）
...
| msmodelslim 未安装 | 安装后重试，或上报编排层 |
| 量化失败（exit code ≠ 0） | 上报编排层：命令 + exit code + stderr 摘要，询问继续 / 换 device / 用现有产物 |
| OOM | 换 device（优先单卡），或上报编排层 |
| 产物校验失败（缺 debug_info / optional） | 检查 `--debug` 是否生效、yaml 是否含对应算法，重新复现 |

#### 约束（步骤 0）

- **错误即停**：量化命令失败立即中止，不兜底续跑
- **单轮单次**：每次只执行一次复现量化
- **config_path 模式**：复现使用 `--config-path`（自主获取的 yaml），与 `--quant-type` 互斥
- **device 单卡优先**：`--device npu:0` 形式
- **磁盘管理**：产物写入 `save_path`，不主动清理历史产物
...
msprobe compare -tp <workdir>/dump_quant/step0 -gp <workdir>/dump_fp/step0 \
  -o <workdir>/compare_result -c cos,max_diff
...
从 compare 结果中筛出**所有 COS 相似度 < 0.95 的激活**（module 的 input/output tensor），按执行顺序列出完整列表（**不限于首个**）：

```
异常激活列表（COS < 0.95，共 N 个）:
1. model.language_model.layers.12.mlp.down_proj.linear [output]  COS=0.8765
2. model.language_model.layers.13.mlp.gate_proj [output]  COS=0.9210
...
逆变换信息: 逆旋转 right 56 / left 28 | 逆抑制 84 个内部 Linear
可视化: tensorboard --logdir <workdir>/compare_result
```

- 阈值默认 0.95（95% 相似度），用户可按需调整
- 无 COS < 0.95 的激活 → 无单点异常，偏差来自量化累积误差。可放宽阈值（如 0.90）、检查是否漏采关键 module、扩展 step/rank 复现。

### 结果输出

向用户输出定位结果报告，包含：

- **所有 COS < 0.95 的异常激活列表**（module + COS 值），或"无异常激活，偏差为累积误差"的结论
- 对比依据证据：dump 路径、COS、compare 结果目录
- 逆变换配置信息（逆旋转/逆抑制范围）
- TensorBoard 可视化入口
...
| 步骤 4-6 | 无 COS < 0.95 的异常激活 | **非错误**：输出"偏差为累积误差"结论 + 建议（放宽阈值 / 补采 module / 扩展 step-rank） |
...
1. 仅基于真实 dump 数据下结论，禁止编造指标、瓶颈或原因；每条结论附 dump 路径、shape 等证据
