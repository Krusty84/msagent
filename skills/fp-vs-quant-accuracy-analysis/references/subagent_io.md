...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `anomaly_module` | string \| null | ✓ | 首个"输入一致、输出不一致"的异常模块路径；未找到为 `null` |
| `module_chain` | string[] | ✓ | 异常模块层级链路 |
| `evidence` | object | ✓ | `{ "input_md5_match": bool, "output_md5_quant": str, "output_md5_fp": str, "cos_similarity": number, "compare_result_dir": str }` |
