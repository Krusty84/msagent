#!/usr/bin/env python3
"""Recommend first-run ms_serviceparam_optimizer search ranges.
...
CONFIG_SKILL_SCRIPT = "skills/ms-serviceparam-optimizer-config/scripts/auto_config.py"
CONFIG_PATH_HINT = "ms_serviceparam_optimizer/ms_serviceparam_optimizer/config.toml"
...
def next_question_for(missing: List[str]) -> str:
    if not missing:
        return ""
    groups = [
        ("hardware.", "请提供硬件信息：单卡显存、world_size、num_per_nodes、num_nodes。"),
        ("model.", "请提供模型 config.json 路径，或 hidden_size、层数、attention heads、KV heads、dtype、最大上下文长度。"),
        ("workload.", "请提供业务负载：平均/最大输入 token 和平均/最大输出 token。"),
        ("target", "请提供优化目标：throughput、ttft、tpot 或 balanced。"),
    ]
    for prefix, question in groups:
        if any(field.startswith(prefix) or field == prefix for field in missing):
            return question
    return f"请补充缺失字段：{', '.join(missing)}。"
...
def kv_capacity(context: Dict[str, Any], parallel: Dict[str, int], model_weight_gb: float) -> Dict[str, int]:
    ...
    max_blocks = math.ceil(to_int(workload["input_len_max"]) / block) + math.ceil(to_int(workload["output_len_max"]) / block)
    avg_blocks = math.ceil(to_int(workload["input_len_avg"]) / block) + math.ceil(to_int(workload["output_len_avg"]) / block)
...
def recommendation(name: str, section: str, dtype: str, value: Any, reason: str,
                   min_value: Any = None, max_value: Any = None,
                   config_position: str = "env", search: bool = True,
                   dtype_param: Any = None, target_field: bool = True) -> Dict[str, Any]:
    item = {
        "section": section,
        "name": name,
        "dtype": dtype,
...
def benchmark_recommendations(capacity: Dict[str, int]) -> List[Dict[str, Any]]:
    ...
    return [
        recommendation("CONCURRENCY", "ais_bench", "int", concurrency_default,
                       "Initial benchmark concurrency follows the estimated safe serving batch.",
                       1, concurrency_hi, "env"),
        recommendation("REQUESTRATE", "ais_bench", "float", max(1, concurrency_default // 2),
                       "Request rate starts below concurrency; optimizer can raise it after measuring throughput.",
                       1, concurrency_hi, "env"),
    ]


def recommend_mindie(context: Dict[str, Any], parallel: Dict[str, int],
                     capacity: Dict[str, int]) -> List[Dict[str, Any]]:
    ...
    items = [
        recommendation("max_batch_size", "mindie", "int", default_batch,
                       "Bounded by estimated KV cache capacity under max input/output length.",
                       max_batch_lo, max_batch_hi, "BackendConfig.ScheduleConfig.maxBatchSize"),
        recommendation("max_prefill_batch_size", "mindie", "ratio", prefill_value,
                       "Ratio of maxBatchSize; lower favors TTFT, higher favors throughput.",
                       prefill_min, prefill_max, "BackendConfig.ScheduleConfig.maxPrefillBatchSize",
                       dtype_param="max_batch_size"),
        recommendation("max_prefill_token", "mindie", "int", max_prefill_tokens,
                       "Total prefill tokens should cover the first-run workload shape.",
                       to_int(workload["input_len_max"]), max(max_prefill_tokens * 2, to_int(workload["input_len_max"])),
                       "BackendConfig.ScheduleConfig.maxPrefillTokens"),
        recommendation("max_queue_deloy_mircroseconds", "mindie", "range", 10000 if target == "ttft" else 100000,
                       "Queue delay trades batching opportunity against TTFT.",
                       500, 1000000, "BackendConfig.ScheduleConfig.maxQueueDelayMicroseconds", dtype_param=100),
        recommendation("support_select_batch", "mindie", "bool", target != "ttft",
                       "Disable for TTFT-first runs; enable for throughput or balanced runs.",
                       0, 1, "BackendConfig.ScheduleConfig.supportSelectBatch", search=target == "balanced"),
        recommendation("prefill_time_ms_per_req", "mindie", "range", 0,
                       "Keep searchable but conservative for first-run schedule exploration.",
                       0, 1000, "BackendConfig.ScheduleConfig.prefillTimeMsPerReq", dtype_param=10),
        recommendation("decode_time_ms_per_req", "mindie", "range", 0,
                       "Keep searchable but conservative for first-run schedule exploration.",
                       0, 1000, "BackendConfig.ScheduleConfig.decodeTimeMsPerReq", dtype_param=10),
        recommendation("max_preempt_count", "mindie", "ratio", 0.1 if target == "throughput" else 0,
                       "Allows limited preemption exploration; keep low for first-run stability.",
                       0, 0.3, "BackendConfig.ScheduleConfig.maxPreemptCount", dtype_param="max_batch_size"),
        recommendation("prefill_policy_type", "mindie", "enum", 0,
                       "First-run schedule policy candidate; keep enum values aligned with MindIE config support.",
                       0, 1, "BackendConfig.ScheduleConfig.prefillPolicyType", dtype_param=[0, 1, 3]),
        recommendation("decode_policy_type", "mindie", "enum", 0,
                       "First-run decode schedule policy candidate; keep enum values aligned with MindIE config support.",
                       0, 1, "BackendConfig.ScheduleConfig.decodePolicyType", dtype_param=[0, 1, 3]),
        recommendation("tp", "mindie", "enum", parallel["tp"],
                       "Selected from attention-head-compatible divisors of world_size.",
                       0, 1, "BackendConfig.ModelDeployConfig.ModelConfig.0.tp",
                       dtype_param=sorted(set([1, parallel["tp"]]))),
        recommendation("dp", "mindie", "int", parallel["dp"],
                       "Derived to satisfy DP * TP * PP <= world_size; keep fixed in first run.",
                       parallel["dp"], parallel["dp"], "BackendConfig.ModelDeployConfig.ModelConfig.0.dp", search=False),
    ]
    if is_moe_model(model):
        world = max(1, to_int(hardware["world_size"], 1))
        expert_count = max(1, to_int(model.get("num_experts") or model.get("num_local_experts") or model.get("n_routed_experts"), world))
        ep_candidates = [item for item in divisors(world) if item <= expert_count]
        moe_ep = ep_candidates[-1] if ep_candidates else 1
        items.extend([
            recommendation("moe_ep", "mindie", "enum", moe_ep,
                           "MoE model detected; explore expert parallelism with divisors of world_size.",
                           0, 1, "BackendConfig.ModelDeployConfig.ModelConfig.0.moe_ep",
                           dtype_param=sorted(set([1, moe_ep]))),
            recommendation("moe_tp", "mindie", "factories", max(1, world // moe_ep),
                           "Derived from moe_ep so moe_ep * moe_tp tracks available parallel resources.",
                           0, 0, "BackendConfig.ModelDeployConfig.ModelConfig.0.moe_tp",
                           dtype_param={"target_name": "moe_ep", "product": world, "dtype": "int"}),
        ])
    return items + benchmark_recommendations(capacity)


def recommend_vllm(context: Dict[str, Any], parallel: Dict[str, int],
                   capacity: Dict[str, int]) -> List[Dict[str, Any]]:
    target = context["target"]
    workload = context["workload"]
    model = context["model"]
    max_model_len = min(to_int(model["max_position_embeddings"]),
                        to_int(workload["input_len_max"]) + to_int(workload["output_len_max"]))
    ...
    batched_tokens_lo = max(to_int(workload["input_len_max"]), to_int(workload["input_len_avg"]) * max(1, max_num_seqs_lo))
    batched_tokens_hi = max(batched_tokens_lo, to_int(workload["input_len_avg"]) * max_num_seqs_hi)
    return [
        recommendation("MAX_MODEL_LEN", "vllm", "int", max_model_len,
                       "Fixed to first-run max input plus max output, capped by model context length.",
                       max_model_len, max_model_len, "env", search=False),
        recommendation("MAX_NUM_SEQS", "vllm", "int", max_num_seqs_value,
                       "Bounded by estimated KV cache capacity under the first-run workload.",
                       max_num_seqs_lo, max_num_seqs_hi, "env"),
        recommendation("MAX_NUM_BATCHED_TOKENS", "vllm", "int", batched_tokens_lo,
                       "Covers expected prefill pressure without starting from an overly wide range.",
                       batched_tokens_lo, batched_tokens_hi, "env"),
        recommendation("TENSOR_PARALLEL_SIZE", "vllm", "enum", parallel["tp"],
                       "Selected from attention-head-compatible divisors of world_size.",
                       0, 1, "env", dtype_param=sorted(set([1, parallel["tp"]]))),
        recommendation("PIPELINE_PARALLEL_SIZE", "vllm", "enum", parallel["pp"],
                       "Keep at 1 for first run unless multi-node or very large model requires PP.",
                       0, 1, "env", dtype_param=sorted(set([1, parallel["pp"]]))),
        recommendation("DATA_PARALLEL_SIZE", "vllm", "int", parallel["dp"],
                       "Derived to satisfy DP * TP * PP <= world_size; wire into startup only if your vLLM mode supports it.",
                       parallel["dp"], parallel["dp"], "env", search=False),
        recommendation("GPU_MEMORY_UTILIZATION", "vllm", "float", 0.9,
                       "Conservative first-run utilization for avoiding startup OOM.",
                       0.85, 0.92, "env"),
        recommendation("BLOCK_SIZE", "vllm", "enum", 16,
                       "KV cache block-size candidate; verify available values with `vllm serve --help` on your environment.",
                       0, 1, "env", dtype_param=[16, 32, 64, 128]),
        default_presence_setting("ENABLE_PREFIX_CACHING",
                                 "Use vLLM's model-aware default; do not pass an explicit flag for first runs."),
        default_presence_setting("ENABLE_CHUNKED_PREFILL",
                                 "Use vLLM's model-aware default; do not pass an explicit flag for first runs."),
        recommendation("COMPILATION_CONFIG", "vllm", "enum", "",
                       "Optional compile/cudagraph candidate; leave empty for the safest first run unless help output confirms support.",
                       0, 1, "env", dtype_param=["", "{\"cudagraph_mode\": \"FULL_DECODE_ONLY\"}"]),
...
def discover_vllm_optional_recommendations(context: Dict[str, Any], help_text: str) -> List[Dict[str, Any]]:
    ...
    if long_prefill and "--max-num-partial-prefills" in text:
        ...
        items.append(discovered_recommendation(
            recommendation("MAX_NUM_PARTIAL_PREFILLS", "vllm", "int", value,
    ...
    if long_prefill and "--long-prefill-token-threshold" in text:
        ...
        items.append(discovered_recommendation(
            recommendation("LONG_PREFILL_TOKEN_THRESHOLD", "vllm", "int", value,
    ...
    if is_truthy(model.get("is_multimodal")) and "--disable-chunked-mm-input" in text:
        items.append(discovered_recommendation(
            fixed_presence_flag("DISABLE_CHUNKED_MM_INPUT", "--disable-chunked-mm-input",
    ...
    if is_moe_model(model) and "--enable-expert-parallel" in text:
        items.append(discovered_recommendation(
            fixed_presence_flag("ENABLE_EXPERT_PARALLEL", "--enable-expert-parallel",
...
def normalized_target_field(item: Dict[str, Any]) -> Dict[str, Any]:
    keys = ("section", "name", "config_position", "dtype", "min", "max", "value", "dtype_param", "search",
            "target_field", "reason", "source", "optional", "placeholder_hint")
...
def command_for_config_skill(item: Dict[str, Any]) -> Optional[str]:
    ...
    if section not in {"mindie", "vllm", "evalscopeperf"}:
        return None
    if section == "vllm" and item["name"] in VLLM_BUILTIN_COMMAND_FIELDS:
        return None

    mode = "--add-search-param"
    args = ["python", CONFIG_SKILL_SCRIPT, mode, "--engine", section, "--param-name", item["name"],
            "--config-position", item.get("config_position", "env"), "--dtype", item["dtype"]]
...
def build_config_skill_handoff(engine: str, recs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    unsupported = [item["name"] for item in recs if item["section"] not in {"mindie", "vllm", "evalscopeperf"}]
    notes = [
    ...
    if unsupported:
        notes.append(f"Current config skill CLI does not directly manage these sections: {', '.join(sorted(set(unsupported)))}.")
    if engine == "vllm":
        notes.append("Set vllm.command.others to include vllm_command_others placeholders before running optimizer.")
    return {
        "version": 1,
        "consumer_skill": "ms-serviceparam-optimizer-config",
...
def target_field_toml(engine: str, item: Dict[str, Any]) -> str:
    lines = [f"[[{engine}.target_field]]", f"name = {toml_value(item['name'])}",
...
def build_toml(engine: str, benchmark: str, recs: List[Dict[str, Any]], world_size: int) -> str:
    ...
    sections = [
        "# Generated by ms-serviceparam-optimizer-param-recommend. Review before applying.",
...
def recommend(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if engine == "mindie":
        ...
        discovered_recs = []
        command = "msserviceprofiler optimizer -e mindie -b ais_bench"
    else:
        ...
        command = "msserviceprofiler optimizer -e vllm -b ais_bench"
    ...
    return {
        ...
        "benchmark_policy": benchmark,
        "assumptions": [
            f"benchmark_policy defaults to {DEFAULT_BENCHMARK}" if "benchmark_policy" not in context else f"benchmark_policy = {benchmark}",
            f"model weight is {model_weight_gb:.2f} GB ({weight_source})",
            f"estimated KV cache budget is {capacity['kv_mem_gb']} GB per card",
        ] + load_notes + discovery_notes,
        "parallelism": parallel,
        "constraints": [{"expression": expression, "recommendation": "Prefer equality for first full-card runs when stable."}],
        "discovery": {
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Recommend first-run ms_serviceparam_optimizer parameter ranges.")
