...
CONFIG_SKILL_SCRIPT_PATH = "skills/ms-serviceparam-optimizer-config/scripts/auto_config.py"
...
def assert_handoff_commands_parse(handoff, tmp_path):
    ...
    try:
        ...
        for command in handoff["apply_commands"]:
            ...
            result = subprocess.run(args, cwd=Path(__file__).parents[3], text=True, capture_output=True, encoding="utf-8")
...
def test_vllm_recommendation_defaults_to_ais_bench_and_parallel_constraint(tmp_path):
    ...
    assert handoff["consumer_skill"] == "ms-serviceparam-optimizer-config"
...
def test_mindie_recommendation_includes_batch_fields_and_ais_bench(tmp_path):
    ...
    assert "--config-position BackendConfig.ScheduleConfig.maxBatchSize" in result["config_skill_handoff"]["apply_commands"][0]
