...
5. **兼容性试加载（必须执行）**：
   - 当模型目录包含 `modeling_*.py`（含权重目录内自带实现）时，必须尝试加载模型类以验证与当前 `transformers` 版本兼容性。
   - 若出现因 `transformers` 版本不匹配导致的加载失败，可提出版本切换建议。
   - **切换 `transformers` 版本前，必须先向用户说明原因、目标版本与预期影响，并获得用户明确确认。未确认不得切换。**
...
4. 确定影响适配的特征：
   ...
   - MTP 结构实现可得性与权重处理风险
5. 尝试模型类加载并记录兼容性结论：
   - 记录加载成功/失败、失败栈关键信息、当前 `transformers` 版本。
   - 若建议切换版本，先请求用户确认，再执行后续操作。
6. 产出结构化分析结果（参考下方模板）。
7. 给出后续动作：
...
- 若模型类试加载失败且判断与 `transformers` 版本相关，必须先向用户确认再进行版本切换；未获确认时应将其列为阻塞项。
