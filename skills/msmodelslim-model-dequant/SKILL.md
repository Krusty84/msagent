...
以下操作会直接修改用户模型目录下的文件，**必须在执行前获得用户明确确认**：

- 修改模型权重文件（如 `*.safetensors`）——包括反量化回写、dtype 提升等
- 修改模型目录下的 `config.json`（如删除量化字段）
- 运行任何会写入模型目录的转换脚本（如 `convert_*_to_bf16.py`）

执行前必须向用户展示：
1. 将被修改的文件路径列表
2. 修改内容摘要（如"将 FP8 权重反量化为 BF16 并回写"）
3. 明确询问"是否确认执行？"

**禁止在未经用户确认的情况下修改模型目录下的任何文件。** 若用户拒绝，应提供替代方案（如将结果写入临时目录）。
