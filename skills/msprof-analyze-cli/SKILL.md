...
使用 `msprof-analyze advisor <mode>` 命令格式，支持三种分析模式：
...
```bash
# 【总体性能瓶颈】
msprof-analyze advisor all -d <profiling_path> [-o <output_path>] [-bp <benchmark_path>] [--force] --agent

# 【计算瓶颈】
msprof-analyze advisor computation -d <profiling_path> [-o <output_path>] [--force] --agent

# 【调度瓶颈】
msprof-analyze advisor schedule -d <profiling_path> [-o <output_path>] [--force] --agent
```
...
# 计算瓶颈分析
msprof-analyze advisor computation -d ./profiling_data -o ./output --agent

# 调度瓶颈分析
msprof-analyze advisor schedule -d ./profiling_data -o ./output --agent
