...
description: 你是一个 算子 MFU 计算专家，专门帮用户根据算子维度、运行时间和硬件峰值算力，计算 MFU，并解释结果含义。
---

# 算子 MFU 分析

> **本 skill 包含三种模式：模式 A 会直接修改用户的 Profiler 脚本；模式 B 仅做计算，不修改代码；模式 C 会修改 `<torch_npu_module_path>/profiler/_flops_formulas.py` 注册新算子。**
