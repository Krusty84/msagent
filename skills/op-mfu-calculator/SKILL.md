...
description: 基于 msprof-analyze 工具的昇腾 NPU 算子 MFU 分析技能。支持三种场景：(1) 用户已有 Ascend PyTorch Profiler 采集脚本，直接修改脚本补齐采集配置并用 msprof-analyze 解析 MFU；(2) 用户想手动计算某个算子的 MFU，无需 Profiling 数据，通过算子公式表确定 FLOPs 后直接计算；(3) 用户需要为未注册的新算子扩展 FLOPs 公式。触发场景：算子性能分析、MFU 瓶颈定位、模型计算效率评估。
---
...
- **分析模式**：
  ...
  - **模式 B**：用户想手动计算某个算子的 MFU → 通过算子公式表确定 FLOPs 后直接计算
  - **模式 C**：用户需要为未注册的新算子扩展 FLOPs 公式 → 注册到 `_flops_formulas.py`、采集验证
...
>
> **强制要求**：无论根据上方的决策树推理出哪个模式，在开始执行之前，**必须先执行以下确认步骤，不得跳过**：
> 1. 简要列出三种模式，说明各自适用场景
> 2. 根据用户需求说明你认为适合走哪个模式及原因
> 3. **请用户明确确认**：得到确认后才能按对应模式执行；用户如有不同意见则按其意愿调整
...
- [ ] 已通过 `_flops_formulas.py` 文件确定 FLOPs 公式并注册
- [ ] 已展示修改的文件路径和修改内容
