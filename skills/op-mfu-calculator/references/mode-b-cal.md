...
2. **op-plugin 源码检索**：公式表中无法确定时，在 `https://gitcode.com/Ascend/op-plugin` 仓库中按以下步骤检索：
   ...
   - **第三步**：如果存在 `FLOP_COUNT` 调用，提取计数函数名（如 `mm_flop`、`bmm_flop`、`flash_attention_forward_flop`），然后在 torch_npu 仓库的 `csrc/flopcount/FlopCounter.cpp` 中查找该函数的具体 FLOPs 计算公式
