...
2. **用户提供算子实现代码**：在线文件中也无法找到时，向用户说明目前有两种途径可以继续：一是**用户直接提供算子的实现代码或源码链接**，我们可以从中推导 FLOPs 公式；二是**通过 op-plugin 源码检索**，需要先 `git clone https://gitcode.com/Ascend/op-plugin` 到本地。

3. **op-plugin 源码检索**（需先 `git clone https://gitcode.com/Ascend/op-plugin` 仓库到当前目录；如已克隆过则直接复用）：在 op-plugin 仓库中按以下步骤检索：
   ...
   - **第二步**：根据算子的计算逻辑（如矩阵乘维度、分组策略、融合操作等）手动推导 FLOPs 公式
   - **第三步**：如果 op-plugin 仓库中无法检索到该算子的实现代码，直接告知用户当前无法确定该算子的 FLOPs 公式，不要给出不确定的猜测。
