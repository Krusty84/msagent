...
> 当用户明确要计算某个**未在算子 FLOPs 公式表中收录**的算子 MFU，或需要为新算子注册 FLOPs 公式以便 msprof-analyze 能识别时，按此流程处理。
...
> **注意**：`_flops_formulas.py` 位于 torch_npu 安装目录下（非项目工作目录），且执行采集命令需要系统权限。在 auto 模式下，Agent 可能无法自动修改该文件或运行命令。如果遇到权限不足或操作失败的情况，**请提示用户手动执行或者退出auto模式**。
...
新增公式函数前，**先阅读 `_flops_formulas.py` 文件中已有的公式函数代码，参考其代码风格和入参处理模式**，然后仿照已有代码编写。
...
采集完成后，DB 文件路径通配符为 `<on_trace_ready输出目录>/*_ascend_pt/ASCEND_PROFILER_OUTPUT/ascend_pytorch_profiler*.db`（多卡场景下任选一个 rank 的 DB 文件确认即可）。确认该文件存在后，运行以下 SQL 查询确认新算子的 FLOPs 是否正确打点：
...
运行 msprof-analyze 前，先确认其版本是否支持 `operator_mfu`（详见 [mode-a-profiling.md](mode-a-profiling.md#33-版本检查确认-msprof-analyze-支持-operator_mfu) 的版本检查步骤）。确认版本无误后，**直接按模式 A 的流程运行 `msprof-analyze` 解析 MFU**（详见 [mode-a-profiling.md](mode-a-profiling.md#34-运行-msprof-analyze-解析-mfu)）。
