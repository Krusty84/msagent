...
- **分支 A**：用户已有 profiling 数据（目录里能看到 `*_ascend_pt/.../ascend_pytorch_profiler*.db`）→ 直接跳到「第 4 步：解析」。
...
| ✅    | `profiler_level ≥ Level1`              | 保证 kernel 信息采全，否则 kernel 缺失无法关联                                            |
...
1. **设置新的输出目录**：`on_trace_ready` 的输出目录指向一个**新目录**（如 `./result_<时间戳>`），不要复用残留上次采集 `*_ascend_pt` 子目录或 `cluster_analysis_output` 的旧目录——msprof-analyze 会报错或给出错误结果。**不要清空已有目录**，直接换一个新目录更安全。
