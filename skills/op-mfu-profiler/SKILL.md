...
description: 算子 MFU Profiling 分析专家。帮用户通过 Ascend PyTorch Profiler 采集性能数据，用 msprof-analyze 解析 MFU，分析算子瓶颈。也支持为新算子扩展注册 FLOPs 公式（详见 references）。
---
...
> **附加能力**：如果目标算子尚未在 `_flops_formulas.py` 中注册 FLOPs 公式，本 skill 也支持为其扩展注册（详见 [references/mode-c-extend-operator.md](references/mode-c-extend-operator.md)），msprof-analyze 即可识别该算子的 MFU。
...
- **文件存在**：读取该文件，向用户列出已注册的算子列表（文件中的 `@register_npu_flop` 装饰器对应的 target），方便用户判断目标算子是否已有现成公式。
...
## 扩展算子：注册新算子的 FLOPs 公式

如果目标算子尚未在 `_flops_formulas.py` 中注册 FLOPs 公式，msprof-analyze 将无法解析其 MFU。此时需要为算子扩展 FLOPs 公式。

**注意**：`_flops_formulas.py` 位于 torch_npu 安装目录下（非项目工作目录），且执行采集命令需要系统权限。在 auto 模式下，Agent 可能无法自动修改该文件或运行命令。如果遇到权限不足或操作失败的情况，**请提示用户手动执行或者退出 auto 模式**。
