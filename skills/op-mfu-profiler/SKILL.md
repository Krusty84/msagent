...
│
├─ 用户明确提出要注册/扩展新算子的 FLOPs 公式
│  → 按 [references/extend-ops.md](references/extend-ops.md) 流程执行
│
└─ 用户没有 Profiler 脚本
...
由 `msprof-analyze` 工具承载。执行 `msprof-analyze -m operator_mfu` 后，工具会从 DB 中读取 `MSTX_EVENTS`，和STRING_IDS表关联，解析出每条记录对应的 FLOPs 和算子名称。同时，通过框架 API 到 Device Kernel 的关联关系，拿到对应的 Device kernel 的执行耗时、输入数据类型。芯片理论峰值则从 device 信息中读取 `ai_core_num` 和 `aic_frequency`，再结合数据类型估算得出。最后，将 FLOPs range 时间窗内的 kernel 与对应的 FLOPs 关联，逐个计算出 MFU。
...
- [ ] 已向用户展示已注册算子列表（表格式），并告知可以扩展未注册算子的 FLOPs 公式
