...
按 SKILL.md「第 4 步：解析」跑解析（默认 db 格式）。db 格式下产物为 `cluster_analysis_output/cluster_analysis.db`，打开其中的 `OperatorMFU` 表，检查**是否出现该新算子**（需直接看 Excel 时可加 `--export_type text` 导出 `OperatorMfu/operator_mfu_kernel_{rank_id}.xlsx`）：
