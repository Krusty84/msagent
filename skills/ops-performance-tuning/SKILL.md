...
编译、运行或采集失败时读取 [troubleshooting.md](references/troubleshooting.md)。禁止：精度失败时继续调优；跨软件栈、SoC、频率、shape、dtype 或 TilingKey 比较；无证据宣称 Bound；照搬案例加速比；一次修改多个无关机制；只展示最快样本或提升 case；批量加载 `references/cases/`。
