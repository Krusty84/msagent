...
- **MCP-only**：所有分析/校验/量化/评测等需要用到modelslim的操作必须通过modelslim提供的MCP工具来完成，禁止使用CLI等方式冒充MCP。
...
- **错误**：执行时使用CLI命令或脚本替代MCP或冒充MCP。
    - 原因：违反了**MCP-only**原则。
    - 正确做法：所有操作必须通过MCP工具完成，禁止使用CLI命令或脚本来替代MCP。
- **错误**：MCP报错后用CLI命令续跑来规避问题。
    - 原因：违反了**MCP-only**原则。
    - 正确做法：应根据**排障和兜底**原则，在MCP报错后进行适当的处理，而不是使用CLI命令来续跑。如果故障无法解决，必须立即中止当前操作，并报出工具名与错误摘要，不应再进行任何形式的续跑或排障。
- **错误**：遇到报错后通过修改源码来规避。
