...
3. 确定本任务允许使用的物理卡，并记录为 `selected_npu_ids`：
   - 用户已指定卡号时，检查其有效性和可用性，不得自动替换、扩展或改用其他卡；
   - 用户未指定卡号时，给出可用卡建议，经用户确认后再继续。
4. 回显设备检查结果和最终确认的 `selected_npu_ids`；确认前不得启动敏感层分析、量化或评测。
...
`selected_npu_ids` 一经确定，敏感层分析、量化和测评均不得超出该列表。
