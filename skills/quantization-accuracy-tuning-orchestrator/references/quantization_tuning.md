...
协议总则见 [subagent_io_protocol.md](./subagent_io_protocol.md)。调用 `task` 时，`description` **必须**包含一个 ` ```msagent-io v1 ` JSON 块；`input` 字段按下表填写。
...
回传 `output` 必填：`practice_path`，`validation: { ok, valid, errors }`
...
回传 `output` 必填：`evaluate_config_path`
...
回传 `output` 必填：`success`，`quantized_path`，`exit_code`
...
回传 `output` 必填：`overall_passed`，`datasets: [{ name, score, target, passed }]`
