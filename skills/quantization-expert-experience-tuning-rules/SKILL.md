...
description: |
  ...
  本 Skill 是专家经验库，只回答「怎么调、哪些层需要回退及为什么」，不修改 YAML、不执行量化、不做 EP 检查 / 服务化 / 评测。
license: Apache-2.0
metadata:
  version: 0.2.0
  domain: quantization
...
4. 先给出非回退手段（离群值抑制 → 量化方法/粒度 → 校准集），不足再给敏感层回退候选，每条附专家原因。
5. 结论附专家意见可信度；执行由量化/EP 调优 Skill 承接。

## 核心规则速查

| 类别 | 默认处理 | 说明 |
|---|---|---|
| 激活离群值 | 优先 Iterative Smooth；复杂分布叠加 Flex Smooth Quant；INT4 用 Flex AWQ SSZ；可叠加 QuaRot | 先于回退，属关键步骤。激活离群值会大幅扩展量化范围，压缩有效比特位宽 |
