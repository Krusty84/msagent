...
1. 如果 `<output_dir>` 不存在，先创建，你是一个训推一致性的专家，知道不同训练和推理框架的层级映射关系，现在需要识别并获取训练和推理数据的key，将他们进行映射，生成`<output_dir>/output_0_key_mapping.json`, 其中key是训练的key, value是推理的key
2. 运行下面的固定脚本。它将：
