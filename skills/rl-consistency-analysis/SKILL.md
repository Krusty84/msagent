...
使用此技能的场景：
- `dump-module-mapping-value-compare` 已运行，但剩余疑点仍需专家级分析
...
1. 运行下面的固定脚本。它将：
   - 调用同级的 `dump-module-mapping-value-compare` 技能重新生成 `output_1` 到 `output_4`
...
此技能期望同级目录 `dump-module-mapping-value-compare/` 存在于同一父目录下。
