...
EPS = 1e-12
MODULE_PREFIX_PATTEN = r"^Module(.\d+|.module|)*.module."
MODEL_PREFIX_PATTEN = r"^Module(.\d+|.model|)*.model."
MODULE_OR_MODEL_PREFIX_PATTEN = f"{MODEL_PREFIX_PATTEN}|{MODEL_PREFIX_PATTEN}"
...
def detect_side(train_path: Path, rollout_path: Path, train_dump: Dict[str, Any], rollout_dump: Dict[str, Any]) -> Tuple[str, str]:
    ...
    def score(dump: Dict[str, Any], path: Path) -> int:
        ...
        if any(re.match(MODULE_PREFIX_PATTEN, k) for k in keys):
            s -= 1
        if any(re.match(MODEL_PREFIX_PATTEN, k) for k in keys):
            s += 1
        return s
...
def fragment_for_output2(key: str) -> str:
    ...
    s = re.sub(MODULE_OR_MODEL_PREFIX_PATTEN, "", s)
...
def normalize_key_for_match(key: str) -> str:
    ...
    s = re.sub(r"^Module(\.\d+|\.module|\.model)*\.", "", s)
