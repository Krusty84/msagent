...
def _session_from_runtime(runtime: Any) -> Any:
    ...
    if session is None or not getattr(session, "loop_worker_running", False):
...
async def add_loop_task(*, schedule: str, prompt: str, mode: str = "once", runtime: Any = None) -> str:
    ...
    if normalized_mode == "once":
        ...
        interval_seconds = 0
        schedule_expr = (
            parse_loop_daily(schedule) if schedule.lower().startswith("daily ") else parse_loop_cron(schedule)
        )
