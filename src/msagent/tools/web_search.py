...
async def _fetch_page_text(
    ...
    if (
        ...
        raise ToolException(
            f"Web fetch only supports text/html or text/plain content, got: {content_type}"
        )
...
def _filter_blocks(
    ...
    *,
    ...
    filtered = (
        [block for block in blocks if _selector_matches(block, selector)]
        if selector
        else list(blocks)
    )
    if query:
        filtered = [
            ...
            if _query_matches(str(block.get("text") or ""), query)
        ]
    return filtered
...
def _filter_plain_text(text: str, query: str) -> str:
    return _clean_multiline_text(
        "\n\n".join(
            ...
            if block.strip() and _query_matches(block, query)
        )
    )
