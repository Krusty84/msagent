...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "..", "..",
                                "skills", "compare-result-analyzer", "scripts"))
...
def _rc(prefix, direction, inp_nre, out_nre, jump=0.0, inp_mean=None,
        out_mean=None, category="ROOT_CAUSE", trigger="", is_param=False,
        param_grad_output=False, no_downstream=False, inheritance=None):
    """构造传播分类条目元组（与 propagation_analysis 输出格式一致）。"""
    return (prefix, direction, inp_nre, out_nre, jump, inp_mean, out_mean,
            category, trigger, is_param, param_grad_output, no_downstream, inheritance)


def _op_group(prefix, direction, worst_in=None, worst_out=None,
              row_range=None, params=None):
    """构造 op_groups 条目。"""
    return {
        'prefix': prefix, 'direction': direction,
        'worst_input_nre': worst_in, 'worst_output_nre': worst_out,
        'row_range': row_range or [0, 0], 'params': params or {},
    }
...
class TestPrecomputeParamGradThreeCategory(unittest.TestCase):
    def test_classifies_param_grad(self):
        ...
        all_prop = {
            "root_cause": [
                _rc("Module.A.parameters_grad.0", "backward", None, 10.0,
                    jump=10.0, param_grad_output=True),
                _rc("Module.B.parameters_grad.0", "backward", None, 5.0,
                    jump=5.0, param_grad_output=True),
            ],
        }
        groups = [
        ...
        self.assertEqual(result["isolated_large_nre"][0]["prefix"],
                         "Module.A.parameters_grad.0")
        self.assertEqual(result["execution_order_first"][0]["prefix"],
                         "Module.B.parameters_grad.0")
...
class TestMergePoolTop(unittest.TestCase):
    def test_dedup_by_prefix_direction(self):
        fwd = [{"prefix": "M", "direction": "forward", "row_start": 0,
                "abs_magnitude": 1.0, "tagging": "none",
                "is_true_root_cause_feature": False}]
        fwd2 = [{"prefix": "M", "direction": "forward", "row_start": 0,
                 "abs_magnitude": 2.0, "tagging": "none",
                 "is_true_root_cause_feature": False}]
...
class TestMain(unittest.TestCase):
    def setUp(self):
        ...
        header = ("NPU Name,Bench Name,NormRelativeErr,MeanRelativeErr,"
    ...
    def test_main_keep_only_zero_rows(self):
        ...
        expected = os.path.join(
            out_dir, "compare_result_nonexistent_kw_xyz_result.json")
...
def main_shim_keep_only(csv_path, keyword, output_path=None):
    ...
    old_argv = sys.argv
    argv = ["analyze_stat.py", csv_path, "--format", "json",
            "--keep-only", keyword]
    if output_path:
        argv += ["-o", output_path]
    sys.argv = argv
