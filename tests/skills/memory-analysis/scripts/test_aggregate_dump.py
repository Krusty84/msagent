...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "..", "..",
                                "skills", "memory-analysis", "scripts"))
...
FULL_HEADER = ["ID", "Event", "Event Type", "Name", "Timestamp(ns)",
...
class TestIterEvents(unittest.TestCase):
    ...
    def test_with_stack_carries_stack_column(self):
        ...
        write_csv(path, FULL_HEADER, [
            [1, "MALLOC", "HAL", "", 100, 1, 1, 0, "0x1",
             "{size:64,owner:PTA@weight}", "Module.load:<module>", ""],
        ])
    ...
    def test_missing_columns_safe(self):
        ...
        write_csv(path, ["ID", "Event", "Timestamp(ns)", "Attr"], [
...
class TestTrendBucketClamp(unittest.TestCase):
    ...
    def test_total_earlier_than_used_no_wrap(self):
        ...
        write_csv(path, FULL_HEADER, [
            ...
            [1, "MALLOC", "HAL", "", 1000, 1, 1, 0, "0xa",
             "{size:1048576,used:-1,total:999}"],
            [2, "MALLOC", "HAL", "", 2000, 1, 1, 0, "0xb",
             "{size:1048576,used:500,total:999}"],
            [3, "FREE", "HAL", "", 3000, 1, 1, 0, "0xa",
             "{used:500,total:999}"],
        ])
        buf = io.StringIO()
        ...
        bucket0 = next(l for l in out.splitlines() if l.startswith("0 |"))
