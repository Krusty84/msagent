...
class TestPickRootCoordinate(unittest.TestCase):
    def test_no_baseline_picks_max_delta(self):
        anomalies = [
            {'rank': 3, 'target_name': 'a', 'norm': 30.0, 'delta': 3.0,
             'micro_step': 1, 'optimizer_step': 0, 'deviation_ratio': 3.0},
            {'rank': 1, 'target_name': 'a', 'norm': 10.0, 'delta': 5.0,
             'micro_step': 2, 'optimizer_step': 1, 'deviation_ratio': 5.0},
            {'rank': 2, 'target_name': 'a', 'norm': 20.0, 'delta': 8.0,
             'micro_step': 3, 'optimizer_step': 1, 'deviation_ratio': 8.0},
        ]
...
class TestNormalizeOpKey(unittest.TestCase):
    ...
    def test_flash_fused_attention(self):
        self.assertEqual(normalize_op_key('mlp.FlashAttention.forward.1'),
                         'mlp.Attention.forward')
