...
@pytest.mark.asyncio
async def test_should_prefer_search_mcp_requires_valid_tavily_key(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_resolve_tavily_api_key_reads_env_placeholders(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_probe_tavily_api_key_uses_usage_endpoint_and_caches_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(factory_module, "_TAVILY_KEY_VALIDATION_CACHE", {})
