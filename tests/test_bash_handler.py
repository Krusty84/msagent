...
async def test_bash_dispatcher_executes_command_with_working_dir(
    ...
    printed: list[str] = []
    monkeypatch.setattr(bash_module.console.console, "print", lambda *args, **kwargs: printed.append(str(args[0])) if args else None)
...
@pytest.mark.asyncio
async def test_bash_dispatcher_prints_stderr_on_nonzero_exit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = BashDispatcher(_build_session(tmp_path))
...
@pytest.mark.asyncio
async def test_bash_dispatcher_prints_stdout_only_on_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = BashDispatcher(_build_session(tmp_path))
    ...
    monkeypatch.setattr(bash_module, "execute_bash_command", fake_execute)
    monkeypatch.setattr(bash_module.console.console, "print", lambda *args, **kwargs: stdout_printed.append(str(args[0])) if args else None)
    monkeypatch.setattr(bash_module.console, "print_error", lambda *_args: None)
    monkeypatch.setattr(bash_module.console, "print", lambda *_args, **_kwargs: None)
