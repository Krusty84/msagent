...
async def test_fetch_skills_returns_display_name_and_filters(tmp_path: Path) -> None:
    skills = [
        Skill(
            name="ascend-cluster-fast-slow-rank-analysis",
            description="Detect slow ranks in distributed runs",
            category="analysis",
            path=tmp_path / "analysis" / "ascend-cluster-fast-slow-rank-analysis" / "SKILL.md",
        ),
        Skill(
    ...
    assert payload == [
        {
            "display_name": "analysis/ascend-cluster-fast-slow-rank-analysis",
            "category": "analysis",
            "name": "ascend-cluster-fast-slow-rank-analysis",
            "description": "Detect slow ranks in distributed runs",
        },
