...
@pytest.mark.asyncio
async def test_hitl_returns_none_when_action_requests_is_empty(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
...
@pytest.mark.asyncio
async def test_hitl_persists_always_approve_selection(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
    cfg = ToolApprovalConfig()
...
@pytest.mark.asyncio
async def test_hitl_auto_rejects_when_policy_is_always_reject(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
    cfg = ToolApprovalConfig()
...
@pytest.mark.asyncio
async def test_hitl_handles_non_dict_tool_args_gracefully(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
    cfg = ToolApprovalConfig()
...
@pytest.mark.asyncio
async def test_hitl_prompt_returns_none_cancels_entire_session(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
    cfg = ToolApprovalConfig()
...
@pytest.mark.asyncio
async def test_interrupt_handler_returns_none_on_exception(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    handler = InterruptHandler(_build_session(tmp_path))
    monkeypatch.setattr(interrupts_module.console, "print_error", lambda *_args: None)
