...
def test_loop_handler_adds_daily_cron_task(tmp_path: Path) -> None:
    async def _run() -> None:
        session = SimpleNamespace(
            context=SimpleNamespace(working_dir=tmp_path),
            loop_tasks=LoopTaskManager(),
        ...
        await handler.handle(["daily", "06:00", "analyze", "data.json"])

        tasks = await session.loop_tasks.list()
        assert len(tasks) == 1
        assert tasks[0].kind is LoopTaskKind.CRON
        assert tasks[0].schedule_expr == "0 6 * * *"
        assert tasks[0].prompt == "analyze data.json"
