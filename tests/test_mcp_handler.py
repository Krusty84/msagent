...
@pytest.mark.asyncio
async def test_mcp_handler_reports_no_servers_configured(
...
@pytest.mark.asyncio
async def test_mcp_handler_skips_reload_when_no_modification(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    session = _build_session(tmp_path)
    server_a = MCPServerConfig(command="node", args=["server-a.js"], enabled=True)
...
@pytest.mark.asyncio
async def test_mcp_handler_handles_load_exception(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    errors: list[str] = []
    monkeypatch.setattr(mcp_module.console, "print_error", errors.append)
...
def test_format_server_list_highlights_selected_server() -> None:
    ...
    styles = [fragment[0] for fragment in formatted]
