...
@pytest.mark.asyncio
async def test_tools_handler_reports_no_tools_available(
...
def test_format_tool_list_shows_tool_names() -> None:
    ...
    formatted = ToolsHandler._format_tool_list(tools, selected_index=0, expanded_indices=set(), scroll_offset=0, window_size=10)
...
def test_format_tool_list_highlights_selected_tool() -> None:
    ...
    formatted = ToolsHandler._format_tool_list(tools, selected_index=1, expanded_indices=set(), scroll_offset=0, window_size=10)
...
@pytest.mark.asyncio
async def test_tools_handler_handles_exception_gracefully(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    errors: list[str] = []
    monkeypatch.setattr(tools_module.console, "print_error", errors.append)
