...
@pytest.mark.asyncio
async def test_web_search_reports_search_failures(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        web_search_module,
        "_run_duckduckgo_search",
        AsyncMock(side_effect=ToolException("DuckDuckGo search timed out after 30s")),
    )

    result = await web_search.coroutine(query="deepagents")

    assert "Unable to complete web search for query: deepagents" in result
    assert "reason=DuckDuckGo search timed out after 30s" in result
