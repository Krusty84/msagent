...
@pytest.mark.asyncio
async def test_fetch_page_text_extracts_and_truncates_html(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    title, content, truncated = await _fetch_page_text(
        "https://example.com/docs", 5, extract_mode="text"
    )
...
async def test_search_results_with_provider_falls_back_when_tavily_fails(
    ...
    monkeypatch.setattr(
        ...
        "_fetch_duckduckgo_html",
        AsyncMock(
            return_value="""<a class="result__a" href="https://docs.python.org/3/">Python Docs</a>"""
        ),
    )
