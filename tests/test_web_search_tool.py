...
@pytest.mark.asyncio
async def test_extract_with_tavily_requests_markdown_content(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured_payload = {}
    captured_headers = {}
    ...
    content = await _extract_with_tavily(
        url="https://example.com/docs",
        api_key="test-key",
...
@pytest.mark.asyncio
async def test_fetch_page_text_uses_tavily_extract_when_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    web_search_module._extract_with_tavily.assert_awaited_once_with(
        url="https://example.com/docs",
        api_key="test-key",
        extract_mode="markdown",
        query="Useful",
