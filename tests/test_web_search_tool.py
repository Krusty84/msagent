...
from msagent.tools.web_search import (
    ...
    _extract_relevant_snippets_with_match,
    _extract_results,
    _fetch_duckduckgo_html,
    _fetch_url_text,
    _filter_results,
    _format_fetch_failure_message,
...
async def test_search_result_urls_uses_duckduckgo(
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=(
                '<a class="result__a" href="https://github.com/langchain-ai/deepagents">'
                "deepagents</a>"
            )
        ),
    )
...
@pytest.mark.asyncio
async def test_web_fetch_reports_timeout_with_no_retry_message(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        web_search_module,
        "_fetch_url_text",
    ...
    assert "Unable to fetch usable web page content from: https://example.com/docs" in result
...
def test_extract_relevant_snippets_with_match_reports_no_hit() -> None:
    result, matched = _extract_relevant_snippets_with_match(
        "Alpha paragraph.\n\nBeta paragraph.",
        "useful",
        max_chars=1000,
    )
...
def test_format_fetch_failure_message_includes_no_retry_suffix() -> None:
    message = _format_fetch_failure_message(
        url="https://example.com/docs",
        failure=FetchFailure(reason="page contains no readable text content"),
        query="install",
    )

    assert "reason=page contains no readable text content" in message
    assert "query=install" in message
    assert "do not retry web_search or web_fetch again for this request" in message
