...
async def test_dispatch_legacy_command_reports_error_when_command_is_unknown(
    ...
    result = await legacy_module.dispatch_legacy_command(
        argparse.Namespace(version=False, cli_command="removed")
    )
...
async def test_handle_config_routes_to_show_when_no_update_option_is_provided(
    ...
    monkeypatch.setattr(
        legacy_module.initializer, "get_registry", lambda _path: registry
    )
...
async def test_handle_config_rejects_provider_when_provider_is_unsupported(
    ...
    result = await legacy_module._handle_config(
        _config_args(tmp_path, llm_provider="unsupported")
    )
...
async def test_handle_config_persists_model_and_process_key_when_provider_update_is_valid(
    ...
    saved = yaml.safe_load(
        (tmp_path / legacy_module.CONFIG_LLMS_FILE_NAME).read_text(encoding="utf-8")
    )["llms"][0]
    ...
    assert "temporary-secret" not in (
        tmp_path / legacy_module.CONFIG_LLMS_FILE_NAME
...
async def test_show_config_renders_provider_key_status_and_mcp_counts_when_servers_exist(
    ...
    servers = {
        "enabled": SimpleNamespace(
            command="python", url=None, args=["server.py"], enabled=True
        ),
        "disabled": SimpleNamespace(
            command=None, url="https://mcp.test", args=[], enabled=False
        ),
    }
    registry = SimpleNamespace(
        get_agent=AsyncMock(return_value=agent),
