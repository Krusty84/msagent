...
def test_wrap_html_in_code_blocks_wraps_html_block_when_block_is_complete() -> None:
    ...
    assert (
        wrap_html_in_code_blocks(content)
        == "```html\n<section>\n<p>Details</p>\n</section>\n```"
    )


def test_wrap_html_in_code_blocks_wraps_only_html_line_when_block_contains_plain_text() -> (
    None
):
    content = "<details>\nplain explanation\n</details>"
...
def test_wrap_html_in_code_blocks_wraps_inline_html_when_html_appears_in_paragraph() -> (
    None
):
...
def test_wrap_html_in_code_blocks_preserves_fenced_code_when_code_contains_html() -> (
    None
):
    content = "```html\n<div>example</div>\n```"
...
def test_wrap_html_in_code_blocks_preserves_indented_code_when_code_contains_html() -> (
    None
):
