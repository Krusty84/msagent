...
def test_extract_thinking_and_text_from_blocks_separates_provider_blocks_when_formats_are_mixed() -> (
    None
):
    blocks = [
        "plain text",
...
def test_extract_thinking_from_metadata_returns_text_when_thinking_payload_is_mapping() -> (
    None
):
    message = AIMessage(
        content="answer", additional_kwargs={"thinking": {"text": "metadata thought"}}
    )

    assert Renderer._extract_thinking_from_metadata(message) == "metadata thought"


def test_extract_thinking_from_metadata_returns_none_when_thinking_payload_is_not_mapping() -> (
    None
):
    message = AIMessage(
        content="answer", additional_kwargs={"thinking": "metadata thought"}
    )
...
def test_build_tool_message_display_returns_none_when_preview_and_content_are_blank() -> (
    None
):
...
def test_build_tool_message_display_marks_error_when_tool_status_is_error() -> None:
    message = ToolMessage(
        content="permission denied", tool_call_id="call-1", status="error"
    )
