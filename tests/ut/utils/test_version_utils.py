...
"""
    ...
    monkeypatch.setattr(
        version_module.importlib.resources, "files", lambda package: resource
    )
...
def test_check_for_updates_returns_none_when_pypi_response_is_unsuccessful(
    ...
    monkeypatch.setattr(version_module, "get_version", lambda: "26.1.0")
    monkeypatch.setattr(
        version_module.httpx, "get", lambda *args, **kwargs: _Response(503)
    )
...
def test_check_for_updates_returns_none_when_network_request_fails(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            version_module.httpx.ConnectError("offline")
        ),
    )
